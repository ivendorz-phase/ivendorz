...
| `DataListTable<T>` (+ `stickyFirstColumn`/`rowHeaderFirstColumn` props) | (semantic table) | P-BUY-01, P-BUY-06, P-BUY-09, P-BUY-14 (price lines), **P-BUY-15** (comparison matrix) | 1 workspace (**5 screens**) | **Strongest Watch** — generic; **Doc-7B `data-table` candidate**. Board ruling (M2): evidence-driven — do NOT promote until a 2nd WORKSPACE (Vendor/Public) needs it (→ §4.2). **API now spans TWO shapes:** cursor-paginated listing AND a transposed matrix (sticky `<th scope="row">` first column + keyboard-focusable scroll region) — record both at promotion time. |
...
| `state-display` (Doc-4M state → label/tone) | — | P-BUY-01/06/08/09/**14** | 1 (Buyer) | No (domain mapping). Note: P-BUY-15 reuses only the LABEL (forces neutral tone — R6) |
| `SealedMarker` (inline sealed-until-close cell marker) | Lock icon + text | **P-BUY-14** (summary amount), **P-BUY-15** (comparison sealed cells) | 1 workspace (**2 screens**) | Watch — extracted to dedupe the byte-identical inline marker (governance-sensitive copy, Doc-3 §10.1); promote on 2nd workspace |
| `ComparisonView` (+ co-located/sibling `ComparisonTable`/`ComparisonSummary`/`ComparisonEmpty`, `comparison-row`/`comparison-column` builders) | `DataListTable` (matrix), `SealedMarker`, Card, StatusChip, EmptyState, Money, formatDate, shell PageHeader/Breadcrumbs | P-BUY-15 | 1 (Buyer) | No (comparison-specific) |
