...
// [ESC-7G-A7] CO-MOUNT DEMO: the seed org is HYBRID, so the vendor route group mounts the SAME
// `HYBRID_SHELL_VM` as the `(buyer)` layout — crossing Buying↔Selling shows an IDENTICAL sidebar (no
// swap). `VENDOR_SHELL_VM` remains the single-surface vendor fixture (now with Trust extracted into its
// own read-only section) and feeds the seam's `resolveMountedNavGroups` selection.
import type { ReactNode } from "react";
import { AppShell } from "../_components/shell";
import { HYBRID_SHELL_VM } from "../_components/hybrid/hybrid-shell-vm";

export default function WorkspaceLayout({ children }: { children: ReactNode }) {
  return <AppShell vm={HYBRID_SHELL_VM}>{children}</AppShell>;
}
