...
| `EngagementDocumentDetail` composition | WP-1 Review-A OBS (2026-07-06): LOI is the FOURTH near-verbatim engagement-document `T-DETAILS` view (PO/Challan/WCC/LOI — shared shape = Breadcrumbs + PageHeader + details `DescriptionList` + `EngagementDocumentFileCard` + party-scope note); rule-of-three crossed for the whole view shape. PER-KIND SLOTS, never in the shared shape: the PO approval section (`can_approve_po` is po-only) AND the DF-6 money `Callout` (PO/LOI only — challan/WCC are explicitly non-financial, no money surface; rendering one there would be a governance error, WP-1 Review-A round-2 NEW-3) | buyer engagement docs (4 views) | Team-2 | WP-1 Review-A rounds 1–2 · RV-0022/0038 lineage | next touch on any of the 4 views | S | Candidate |
| **CS document primitives** (`CsSheet`/`CsCompactHead`/`CsSignatures`/A4 print CSS) | WP-2 (2026-07-06): the first fixed-A4 print-document family (`app/(app)/(buyer)/_components/comparative-statement/`) — sheet chrome, repeating letterhead header, wet-ink signature grid, `@page` A4 print rules, content-driven page chunking. Expected 2nd consumer: **M4 engagement-document print views** (LOI/PO/challan/WCC render as official documents too — freeze `COMPARE_SHEET_UX_FREEZE` v1.0 §3 says the CS layout standard is "reused across all RFQs"; the same primitives generalize). Per-document content sections stay document-scoped | buyer CS (single surface today) | Team-2 → Kit owner | WP-2 WP card (`governanceReviews/milestones/wp-2-comparative-statement/`) | first M4 print-view milestone | M | Candidate (single-surface today — registry 2nd-consumer rule) |
