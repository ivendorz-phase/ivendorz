...
| FE-PUB-05 Product Detail | Core Marketplace | P1 | M | Med | ~~H: `ESC-7-API-PRODDETAIL`~~ **RESOLVED 2026-07-03 (owner Board, `R-ESC7-PRODDETAIL-FREEZE`)** | P-PUB-11 | ⬜ **Un-gated 2026-07-03** — `marketplace.get_public_product_detail.v1` (Doc-4D v1.0.3/Doc-5D v1.0.1) + `ADR-025` canonical product URL law (`/marketplace/product/{name-slug}-{uuid}`) + registered `marketplace.public_read_rate_limit` (Doc-3 v1.11) all folded together; retire the interim `search_catalog`-modal at cutover per the frozen build; wire the R9 non-disclosure collapse + breadcrumb deterministic pick rule per the folded patch text — **Team-1's next pull** |
