...
| P-ACC-16 | Plans / catalog | P1 | Ready | 🟡 In Progress | Building — SK-CARD plans catalog at /account/billing; frozen `list_plans`/`get_plan` (BC-BILL-1 read; {plan_id,name,billing_cycle,price,currency,status}); billing-cycle toggle; **entitlements NUMERIC/enum gate features NOT plan-name (Inv #10); plan≠tier; no plan gates matching/award (Doc-3 §11.8)**; Current-plan by entitlement; Select→P-ACC-17; Empty |
