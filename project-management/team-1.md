...
| P-ACC-10 | Permissions reference | P2 | Ready | 🟡 In Progress | Building — read-only `list_permissions` catalog (`{slug,description,space}`, Doc-4C §C7:454); columns permission(slug)·scope(space)·description; permissions by reference (Inv #10); reuses P-ACC-09 catalog (single source, no dup); spec "group"→frozen `space` (no semantic group field) |
