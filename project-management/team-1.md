...
| P-ACC-11 | Delegation grants | P2 | Ready | 🟡 In Progress | Building — T-LISTING `list_delegation_grants` (Doc-4C §C9:655); columns delegator·delegatee·scope(vendor_profile)·status·expiry(valid_to); grant states active/suspended/revoked/expired (Doc-2 §5.10); status filter; New grant + row→detail (P-ACC-12); list-only (no suspend/revoke here); Empty |
