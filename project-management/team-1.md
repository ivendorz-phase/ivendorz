...
| P-ACC-09 | Role editor | P2 | Ready | 🟡 In Progress | Building — create/edit role editor (/new + /[roleId]); frozen `create_role`/`update_role`/`set_role_permissions`/`delete_role` + `list_permissions` catalog (Doc-4C §C7); permissions by SLUG not name-string (Inv #10); system bundles read-only (not renamable/deletable); `[ESC-IDN-SLUG]`; delete-confirm |
