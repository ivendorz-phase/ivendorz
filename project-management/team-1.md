...
| P-ACC-13 | Workflow settings | P2 | Ready | 🟡 In Progress | Building — frozen `update_workflow_settings` (Doc-4C §C11:714): `rfq_approval_mode` enum none/single/multi_step + approval_chain + financial_permissions(threshold) + no-auto-approve (Doc-3 §1.2); Owner/Director scope; jsonb shapes grounded from Doc-3 §12.3 or deferred (don't-coin) |
