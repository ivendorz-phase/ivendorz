...
| P-ACC-17 | Subscription | P1 | Ready | 🟡 In Progress | Building — T-DETAILS subscription at /account/subscription; frozen `get_subscription`/`cancel_subscription`/`list_subscription_events` (BC-BILL-2); §5.7 states pending_payment/active/expired; **cancel = auto_renew=false, NOT immediate expire (A-06); entitlements NUMERIC usage not plan-name (Inv #10)**; events timeline; cancel-confirm; `?state=none` empty; platform-revenue only (no buyer↔vendor money) |
