...
| P-ACC-20 | Platform invoices | P2 | Ready | 🟡 In Progress | Building — T-LISTING at /account/invoices; frozen `list_platform_invoices` (BC-BILL-5); table ref·date·amount·status; **ref = OPAQUE `platform_invoice_id` bare UUID (DD-5) — render opaque mono, NEVER coin INV-… (P-ADM-11 lesson)**; status chip (paid/issued/overdue/void); amounts tabular+currency; **PLATFORM-fee only ≠ M4 trade invoices (Doc-4I H.9 FIXED, R8/DF-6); no escrow/wallet/settlement**; row→P-ACC-21; status filter; Empty |
