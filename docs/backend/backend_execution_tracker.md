...
| **W2-CORE-2** | Outbox dispatch hardening + Inngest wiring (`pending→dispatched→archived`) | CORE-1 ✅ | 🔵B (delta re-verify) | **PASS** (RV-0144) | B-1/B-2/B-3 + A-NIT-1 patched at `7ecd284` (101/16; CAS discrimination proven by revert-fail/restore-pass; lock-barrier race test, no sleeps) | — | `7ecd284` (parent `167ed68`) | 8B · 8F ✅ (101/16) | `[D-5]` on Board channel | Delta-only Team-5 re-verify at `7ecd284` → clean-gate close; **Team-6 N/A re-confirmed by both lenses**; deviation recorded: Prisma-6 removed `$use` → FOR-UPDATE lock-barrier substitution |
