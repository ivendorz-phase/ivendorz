...
> Corpus copy `generatedDocs/Doc-7E_GrowthHub_Patch_v1.0.1.md`, registered in `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`,
> carried **alongside** its unedited frozen base document(s) — **no frozen file edited in place.**
> Origin/provenance: `governanceReviews/Doc-7E_GrowthHub_Patch_v1.0.1_PROPOSAL.md` (the authoring + per-patch
...
# Doc-7E — Additive Patch v1.0.1 (Growth Hub Surface) — Account & Identity Shell
...
| **Version note** | The existing `Doc-7E_Content_Pass{1,2}_Patch_v1.0.1` files are **pre-freeze components of v1.0** (applied inside the frozen effective set); this is the first **series-level** additive patch, producing Doc-7E **v1.0.1** — the distinct `GrowthHub` filename prevents confusion. |
...
| **Depends on** | `Doc-5C_…v1.0.1` (the wire — **review-clean upstream**) · `Doc-4C_…v1.0.3` (§PROV-EXT — attribution at provisioning) · `Doc-2_Patch_v1.0.10` (states) · `Doc-3_…v1.14_GrowthHub` (keys, by effect). **Atomic fold.** |
