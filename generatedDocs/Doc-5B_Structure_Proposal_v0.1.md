...
3. **Carried items are referenced, never resolved.** Doc-4B's open escalations (D-1, D-2, PA-E1, PA-E2) are carried by pointer; Doc-5B resolves none of them.
...
| G5 System Configuration (change + read) | Admin command + read | **Caller-facing HTTP** `POST`/`GET` |
| G6 Feature Flags (set + read) | Admin command + read | **Caller-facing HTTP** `POST`/`GET` |
...
- **R2 — Admin-only actor surface.** M0 caller-facing contracts are Admin (platform-staff) or System; **no tenant-user business operation, no active-organization context, no delegation** (Doc-4B §A1; Doc-5A §7.2/§7.3 — admin carries no org context, §7.4 delegation N/A).
- **R3 — `core` route prefix** (Doc-5A Appendix B.1).
- **R4 — No slug invented.** Admin endpoints bind the existing `staff_super_admin` (Doc-2 §7) pending the D-2 least-privilege patch; Doc-5B invents nothing (Doc-5A App A CHK-5A-154 / Doc-4A §6.4).
...
- **Purpose:** what Doc-5B governs (the M0 HTTP surface) and does not; carry the surface partition table; carry Doc-4B escalations (D-1/D-2/PA-E1/PA-E2) **by pointer**.
...
- **Binds:** `Doc-5A §1`; Doc-4B §A1/§A2. **Detail:** scope + partition tables.

### §2 — Realized Endpoint Inventory
- **Purpose:** the `core`-namespace HTTP surface — one row per caller-facing endpoint (G1/G2/G5/G6): method (§5.2), path grammar (§5.3), actor (Admin/System), success status (§5.5). Every endpoint instantiates the §5.7 template (filled in content).
- **Binds:** `Doc-5A §5`, App B.1 (`core`); Doc-4B G1/G2/G5/G6. **Detail:** inventory table (paths in content pass).

### §3 — Audit Read Surface Realization
- **Purpose:** G1 → `GET` reads; non-disclosure on the wire (§6.3/§7 — audit of protected-fact-gated operations never served to a gated-out party, Doc-4B §A4); pagination/filter (§8); top-level `reference_id` (§6/C-05).
- **Binds:** `Doc-5A §5/§6/§7/§8`; Doc-4B §A4. **Detail:** read-surface realization.

### §4 — Audit Redaction Realization
- **Purpose:** G2 → `POST` named command; idempotency/concurrency carriage (§9); error mapping (§6); the redaction action itself audited (Doc-4B §A5).
- **Binds:** `Doc-5A §5/§6/§9`; Doc-4B §A5. **Detail:** command realization.

### §5 — System Configuration & Feature-Flag Surface Realization
- **Purpose:** G5+G6 → Admin `GET` reads + `POST` change/set commands; POLICY-key boundary (values by key, never literal — Doc-5A §8/§12 · Doc-3 §12); carry the PA-E2 config-governance-ownership flag (Doc-4J vs Doc-4B) **by pointer**, unresolved.
- **Binds:** `Doc-5A §5/§6/§9`; `Doc-3 §12`; Doc-4B §A8/§A9, PA-E2. **Detail:** config/flag realization.

### §6 — Out-of-Wire Boundary (G3 · G4 · G7)
- **Purpose:** declare that G3 (outbox dispatch/archive), G4 (ID/human-ref allocation), and G7 (internal infrastructure obligations) have **no HTTP wire**: System workers and in-process mechanisms invoked within consuming modules' transactions. Implementation is code / Doc-6. **Flag-and-halt if any wire surface is proposed for them** (would be an architecture change).
- **Binds:** Doc-4B §A6/§A7/§A10; `Doc-5A §1.3` (deferral), §10.5/§11 (delivery vs surface). **Detail:** boundary statement only — no realization.

### §7 — Conformance & Carried Items
- **Purpose:** Doc-5B's attestation against Doc-5A **Appendix A** (`CHK-5A-xxx`, the freeze gate); carried Doc-4B escalations (D-1/D-2/PA-E1/PA-E2) registered by pointer; statement that Doc-5B coins nothing.
- **Binds:** `Doc-5A Appendix A`; Doc-4B §A2/§A11. **Detail:** attestation + carried-items register.
...
## Carried items register (by pointer — not resolved here)

| ID (Doc-4B) | Item | Doc-5B handling |
|---|---|---|
| D-1 | Deferred internal-service / event-schema templates | out-of-wire (§6); no template invented |
| D-2 | No least-privilege `staff_*` slug for config/flag/audit-read | binds existing `staff_super_admin`; carried |
| PA-E1 | Missing `core.system_configuration.{outbox,audit,core}.*` POLICY keys | referenced by intended name; flagged for Doc-3 §12.2 patch |
| PA-E2 | Config-change governance ownership (Doc-4J vs Doc-4B) | carried in §5 by pointer; no ownership moved |
