# Doc-4E — RFQ Procurement Engine — Pass-B (Hardening) Part 3 v1.0 (FROZEN) — BC-3 Routing + BC-7 Governance

## BC-3 Routing, Selection & Distribution + BC-7 Routing Governance & Control Plane Hardening (§E6)

| Field | Value |
|---|---|
| Document | Doc-4E — **Pass-B Part 3 v1.0 (FROZEN)** — final immutable Part-3 baseline — Module 3 RFQ Procurement Engine (`rfq` schema) |
...
| Status | **Part-3 FROZEN — final immutable baseline.** Consolidates `Doc-4E_Content_v1.0_PassB_Part3_RoutingGovernance.md` as amended by `Doc-4E_PassB_Part3_Patch_v1.0.md`; certified by `Doc-4E_PassB_Part3_Freeze_Audit_v1.0.md`. Authorized next stage: **Doc-4E_PassB_Part4_v1.0** (BC-4). |
...
| `invitation_id` resolves | 7 REFERENCE | Doc-4A §4.5/§9.5; Doc-2 §10.4 (RFQ Invitation aggregate) | `invitation_id` MUST resolve to an existing `rfq_invitations` record | `NOT_FOUND` (protected-fact collapse, §7.5/§12.4) |
