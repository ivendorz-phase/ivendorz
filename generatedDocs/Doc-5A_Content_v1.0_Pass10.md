...
> **Sync flag — RECONCILED (`PATCH-D5A-0D`):** the `Iv-Api-Version` row records **ownership only** ("Version-carriage header (owned by §12)"). §4.4's classification cell has been aligned to the **identical** wording by `PATCH-D5A-0D`, satisfying the §4.5 normative synchronization requirement (`CHK-5A-153`). No header behavior is defined here; **§12 remains sole owner** of version behavior.
...
> **Governance flag — STRUCT-02 (APPLIED, `PATCH-D5A-STRUCT-02`):** the content header set above excludes the pagination cursor, consistent with the **frozen `Doc-4A §9.6`** (cursor = opaque request parameter, owned by §8) and the §4.4 content registry. The structure §4 purpose-line wording has been synchronized by the additive **`PATCH-D5A-STRUCT-02`** (frozen structure patched by file, merged at consolidation). Wording-sync only — no substantive change.
