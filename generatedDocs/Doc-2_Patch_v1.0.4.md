...
> `generatedDocs/Doc-2_Patch_v1.0.4.md` (producing **Doc-2 v1.0.4**), registered in `00_AUTHORITY_MAP.md`,
> carried **alongside** the unedited frozen `Doc-2_…_v1.0.2` (+ v1.0.3) — **no frozen file edited in place.**
> Origin/provenance: `governanceReviews/Doc-2_Patch_v1.0.4_BuyerProfileAudit_PROPOSAL.md`. **Linked-pair** with
> the Doc-4C realization patch (`Doc-4C_BuyerProfileAuditToken_Patch_v1.0.2`) — frozen together.
>
> **Board ruling (2026-06-30): APPROVED WITH ONE MODIFICATION** — Doc-2 carries **business semantics only**;
> the wire-level audit **token** realization is moved **out of Doc-2** into the Doc-4C realization patch. This
> patch applies that ruling (business actions only; no wire literals).
...
Doc-4B `core.append_audit_record.v1` (unchanged), D7 (exported action constants). **APPROVED & FOLDED into the corpus
(human, Board ruling 2026-06-30); linked-pair with Doc-4C_BuyerProfileAuditToken_Patch_v1.0.2.***
