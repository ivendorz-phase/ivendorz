# Doc-4D_PublicProductDetail_Patch_v1.0.3.md

> **✅ STATUS: APPROVED (human — owner/Board ruling 2026-07-03) + FOLDED into the corpus.**
> This is the corpus copy `generatedDocs/Doc-4D_PublicProductDetail_Patch_v1.0.3.md`,
> registered in `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`, carried **alongside** the unedited
> frozen Doc-4D content passes (v1.0 + `Doc-4D_CanonicalHost_Patch_v1.0.2`) — **no frozen file
> edited in place.**
> Origin/provenance: `governanceReviews/Doc-4D_PublicProductDetail_Patch_v1.0.3_PROPOSAL.md`.
>
> **Linked-pair** with `Doc-5D_PublicProductDetail_Patch_v1.0.1` (wire realization) — the two
> patches are approved and folded **together**, mirroring the Doc-2 v1.0.4 ↔ Doc-4C v1.0.2 and
> Doc-2 v1.0.5 ↔ Doc-4D v1.0.2 linked-pair precedents.
>
...
> (**`R-ESC7-PRODDETAIL-FREEZE`**, human Board, 2026-07-03) — Decisions **2** (coin the composed
> read) and **3** (category path embedded-only). Owner plan-review rulings of 2026-07-03 (two
> adjudication rounds, all findings accepted) are folded and marked **[owner-ruled 2026-07-03]**
> where they refine the resolution text.
>
...
> **[review-adjudicated 2026-07-03]** where they correct resolution text upward to the frozen
> corpus); OBS items recorded, no action.
...
- **Rate Limiting (Doc-4A §19):** throughput-type, `RATE_LIMITED` error class, POLICY-bound. Bound to the
  ...
  patch** — Doc-4A §18.2 gate satisfied at fold time).
...
POLICY key coined (E-2 registers the name only — no value fixed here) · no governance-signal
...
- **`Doc-5D_PublicProductDetail_Patch_v1.0.1` (linked pair):** realizes this contract on HTTP (wire path per
  ...
  `get_spec_library_entry` conformance correction (resolution §I.5.3). **Folded together with this patch.**
...
- **E-3 (`ADR-025_Marketplace_Public_URL_Law`):** canonical product URL law. **Folded together with this
  patch.**
- **`ESC-7-API-PRODDETAIL`:** the E-1 **contract leg** is resolved by this patch — registry pointer updated
  at fold time.
- **FE-PUB-05:** un-gated at this fold — Annex E items E-1/E-2/E-3 have all landed together.

---

*End of Doc-4D_PublicProductDetail_Patch_v1.0.3 — one additive 21.3 Query
(`marketplace.get_public_product_detail.v1`) in BC-MKT-6; coins no verb, no error code, no slug, no event,
no audit action; POLICY registered by the linked E-2 instrument; R9 collapse centralized; trust
beside-never-inside; vendor_slug withheld from the payload and `canonical_url` opaque per owner rulings
2026-07-03. Linked-pair with Doc-5D_PublicProductDetail_Patch_v1.0.1 — approved and folded together.*
