...
> Corpus copy `generatedDocs/Doc-6C_GrowthInvitation_Patch_v1.0.4.md`, registered in `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`,
> carried **alongside** its unedited frozen base document(s) — **no frozen file edited in place.**
> Origin/provenance: `governanceReviews/Doc-6C_GrowthInvitation_Patch_v1.0.4_PROPOSAL.md` (the authoring + per-patch
...
CREATE TABLE identity.growth_invitations (
  ...
  recipient_identifier      text,                                            -- [Doc-2 v1.0.10 §1] targeted only; NULL for link/qr; **GI-3 confined** (see §5 read paths); at-create (GI-2)
  ...
  redemption_count          integer NOT NULL DEFAULT 0,                      -- [Doc-2 v1.0.10 §1] the GI-1 atomic capacity gate (§4); mutable
...
CREATE TABLE identity.invitation_conversions (
  ...
  referrer_organization_id  uuid NOT NULL,                                   -- [Doc-2 v1.0.10 §1] denormalized tenant anchor — the ONE documented denormalization on-table (RLS anchoring without traversal; Rec-2's justified-denorm rule); (at-create = [§2.5 choice])
  ...
  created_at timestamptz NOT NULL DEFAULT now(), updated_at timestamptz NOT NULL DEFAULT now(),
  created_by uuid, updated_by uuid,                                          -- [Doc-6A B.1] the full base-model tuple — Doc-2 v1.0.10's shorthand lists `created_*` only, but B.1 mandates the tuple for ALL tables and the 5.12 bind mutates the row (Review-A GH6C-7 reconciliation)
...
CREATE TRIGGER invitation_conversions_block_atcreate_mutation BEFORE UPDATE ON identity.invitation_conversions
  FOR EACH ROW EXECUTE FUNCTION core.raise_immutable_violation(
    ...
    'id', 'created_at', 'created_by');
    -- [§2.5 choice] (Review-A GH6C-3) — Doc-2 v1.0.10 marks NO conversions column at-create (GI-2 covers the
...
flagged):** no `delivery_reference_id` column exists on either table and **Doc-2 v1.0.10 coins none** —
...
## §6 — POLICY bindings (Doc-3 `…v1.14_GrowthHub` — read, never literal) + seeds + counts

| Key (Doc-3 v1.14) | Used by (here) |
|---|---|
...
1. **Slug seed** — `can_manage_growth_invites` into `identity.permissions` + composition into the O/D/M
   role bundles (indicative defaults, Doc-2 v1.0.10 §3) — the `Doc-6C_Patch_v1.0.1/.3` seed-overlay
   mechanism. **Count assertion 46 → 47** (tenant 37 → 38; staff unchanged 9) — the **v1.0.9 baseline**
   (the folded VBR patch already took 45→46/36→37; `Doc-2_Patch_v1.0.10` §3). *(The VBR train's own
   planned Doc-6C overlay asserts 46 — whichever folds second re-verifies the running total; header
   contingency.)*
2. **POLICY-key seed** — **6 valued** `identity.*` v1.14 keys seeded into `core.system_configuration`
   with the v1.14 proposed start values (Board-adjustable at fold), incl.
   ...
   **`identity.growth_invite_resolve_rate_limit` seeds NO value** (Review-A GH6C-2 / Review-B F2 — the
   v1.14 key commits no number, the v1.11 model; the operational value is set via the authorized
...
- **`[ESC-6-POLICY]`/`[ESC-6-API]`-class flag — the expiry sweep is DOUBLY unrealized (Review-A GH6C-4
  widened; predicted by Doc-3 v1.14 Review-A OBS-1):** the §5.11 `issued → expired` System sweep runs on
...
- **Mini-attestation (Doc-6A Appendix-A bands, additions only):** CHK-6-001 UUIDv7 PKs ✓ · 6-003
  ...
  the §7 sweep flag are the routings ✓ · 6-060/6-061 v1.14 keys seeded (6 valued; rate-limit
...
coined (realizes Doc-2 v1.0.10 + Doc-3 v1.14 verbatim) · □ GI-1 placement documented (service txn; no
