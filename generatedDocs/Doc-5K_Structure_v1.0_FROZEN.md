# Doc-5K — AI Layer (M9 `ai`) API Realization — Canonical Structure v1.0 (FROZEN)

| Field | Value |
|---|---|
| Status | **FROZEN** — canonical Table of Contents for Doc-5K |
| Freeze Date | 2026-06-26 |
| Supersedes | `Doc-5K_Structure_Proposal_v0.1.md` (effective v0.2 — Board pre-authoring findings 5 MINOR + 6 NITPICK incorporated; Independent Hard Review applied, REC-1 reconciled + 3 MINOR + 1 NITPICK resolved; authoring history + findings map + disposition retained there). Freeze readiness certified by `Doc-5K_Structure_Freeze_Audit_v1.0.md` |
...
| Precedent (informational, not authority) | `Doc-5B` (M0) out-of-wire boundary (R1); `Doc-5G` (M5) reconciliation-gate pattern; `Doc-5H` (M6) cross-cutting wire model + route/token discipline + attestation bands |
...
- **R1 — Out-of-wire boundary.** Doc-5K realizes only the caller-facing HTTP surface. **Out-of-wire (→ §5):** the 4 `expire_*` (System TTL/regeneration sweep timers, hard-delete), the 4 `generate_*` (AI-Agent on-demand + System scheduled — **Actor `AI Agent / System`, no caller actor; in-process; out-of-wire — REC-1 resolved**), and the **internal-service leg** of the dual-audience `get_*`/`list_*` reads (consumed in-process by owning modules). **Flag-and-halt if a caller wire is proposed for any.** (Authority `Doc-5A §1.3/§5/§11`; Doc-5B R1 precedent.)
...
>
> **REC-1 (resolved):** verified against the Doc-4K per-contract Identity blocks — `generate_*` carry Actor **`AI Agent / System`** (no User/Admin caller actor) ⇒ **out-of-wire**; `get_*`/`list_*` carry `21.3 · User / System` ⇒ the **User leg is caller-facing** (§4), the System/internal-service leg is out-of-wire (§5); `expire_*` carry `21.5 · System` ⇒ out-of-wire. The get/list internal-service leg is a **mechanism, not a counted contract**. **Final split: 8 caller + 8 out = 16.**
...
| BC-AI-1…4 `generate_recommendation`, `generate_prediction`, `generate_classification`, `generate_similar_vendors` | AI-Agent on-demand + System scheduled (21.4/21.5; Actor AI-Agent/System — no caller actor; in-process) | **§5** out-of-wire |
...
- **Purpose:** Doc-5K's attestation against Doc-5A **Appendix A** (the freeze gate); the carried-items register (DF-AI-1…6 + `[ESC-AI-AUDIT]` / `[ESC-AI-EVENT]` / `[ESC-AI-POLICY]` / `[ESC-AI-SLUG]` + **`[REC-AI-WIRE]`**) by pointer with each named resolution channel; statement that Doc-5K coins nothing and restates no DF-AI rule.
...
| **`[REC-AI-WIRE]`** | Wire classification of `generate_*` + get/list internal leg (REC-1) | **RESOLVED** — `generate_*` Actor = `AI Agent / System` (no User/Admin caller actor) ⇒ out-of-wire; get/list User leg caller-facing (§4), System leg out-of-wire (§5). Verified against Doc-4K Identity blocks; final 8+8. **Reconfirm verbatim at content** | **Satisfied at freeze** |
...
*End of Doc-5K Canonical Structure v1.0 (FROZEN) — structure only. On any conflict, Doc-5A (FROZEN) and the frozen corpus win; flag-and-halt. Authoring history, Board-Findings Map, and Review Disposition retained in `Doc-5K_Structure_Proposal_v0.1.md` (v0.2); freeze readiness certified by `Doc-5K_Structure_Freeze_Audit_v1.0.md`. `[REC-AI-WIRE]` satisfied at freeze (generate_* out-of-wire; final 8+8) — reconfirm verbatim at content. Next: content passes — Pass-1 (§0–§3 + inventory), Pass-2 (§4 reads), Pass-3 (§5–§6 + Appendix A) — each conforming to this structure.*
