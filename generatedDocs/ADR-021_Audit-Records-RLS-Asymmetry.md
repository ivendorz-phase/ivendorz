...
**Status:** **APPROVED — human, 2026-06-30.** Carried into the corpus as the **individual ADR legal record**
(`generatedDocs/ADR-021_Audit-Records-RLS-Asymmetry.md`), alongside the unedited frozen
`generatedDocs/ADR_Compendium_v1.md` — which is **not** edited in place by any AI action (CLAUDE.md §7, rank 1).
**ADR-021** is the next free number (**ADR-019 stays reserved / do-not-backfill** per Compendium §B-9; ADR-020 is
the last used). Registered in `00_AUTHORITY_MAP.md`. **Deferred to a human re-freeze:** consolidating ADR-021 into
a re-issued **ADR Compendium v2** (this carried-alongside legal record + the authority-map registration are the
effective record until then). Origin/provenance: `governanceReviews/ADR-021_Audit-Records-RLS-Asymmetry_PROPOSAL.md`.
...
*ADR-021 (APPROVED — human, 2026-06-30) — extends ADR-009, refines ADR-016, supersedes nothing. Records the
durable rationale of ESC-W2-AUDIT-RLS §7 = R-b; realized by the D2/D3 additive patches (`Doc-4B_AuditAppendRLS_Patch_v1.0.1`
· `Doc-6B_Structure_Additive_Patch_v1.0.1`). Carried as the individual ADR legal record alongside the frozen ADR
Compendium (not edited in place by AI); registered in `00_AUTHORITY_MAP.md`. Consolidation into an ADR Compendium v2
is a deferred human re-freeze.*
