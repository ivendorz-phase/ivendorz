...
**Carried into Doc-6C…6K (per-module gates):** `DR-6-CORE` (M0 owns outbox/audit/id/config) · `DR-6-API` (Doc-5 persistability cross-check) · `DR-6-STATE` (state machines as columns + enforcement) · `[ESC-6-SCHEMA]` (additive Doc-2 patch) · `[ESC-6-POLICY]` (additive Doc-3 §12.2 patch — incl. open **M1 `identity`** namespace, a Doc-6C cross-check) · `[ESC-6-API]` (Board → additive Doc-2 patch). **Doc-6B (M0 `core`) FROZEN 2026-06-26.** **Next:** Doc-6C (M1 `identity`) — carries the open `[ESC-6-POLICY]` identity-namespace question.
...
## 5d. Doc-8 Program (Test & Conformance Realization — STARTED; Doc-8A structure FROZEN)

> The verification sibling of the Doc-5 API / Doc-6 DB / Doc-7 FE programs. **Doc-8A is the metastandard** (the Doc-5A/6A/7A analog); it governs the test-discipline-partitioned realizations Doc-8B…8G via Appendix A (`CHK-8-xxx`). Realizes the full frozen *what*-corpus (Doc-2/3/4 — the test oracle) + the Doc-5/6/7 realization contracts (under test) as deterministic, isolated, CI-gating conformance suites. **Doc-8 is the conformance harness yet subordinate to its oracle** (a downward gate over implementations; upward-subordinate to the corpus — governance §3/§8 rule 5): a red test = code defect (fix it) or corpus defect (`[ESC-8-CORPUS]`, flag-and-halt) — never weaken the assertion.

| File | Purpose |
|------|---------|
| `Doc-8A_Structure_v1.0_FROZEN.md` | **Doc-8A structure FROZEN** — Test & Conformance Realization Metastandard (structure); effective = `Doc-8A_Structure_Proposal_v0.1` + `Doc-8A_Structure_Patch_v0.1.1`. Fixes R1–R12; **test-discipline partition** (Doc-8B Foundation/Harness *(frozen first)* + 8C Contract/API + 8D Persistence/Migration/RLS + 8E Domain/Invariant/State-Machine + 8F Integration/Event-Flow + 8G Frontend/E2E); cross-cutting conformance-concern allocation table (RLS/non-disclosure→8D, firewall/invariant/state→8E — asserted once, composed elsewhere); Appendix A = 9 `CHK-8-xxx` bands (per-suite freeze gate); inherits the RLS positive/negative/cross-tenant byte-equivalence gate (`Doc-6A R8/§4`) + the migration gate (`Doc-6A §11`). **Content passes NOT STARTED.** Coins nothing |
| `Doc-8A_Structure_Proposal_v0.1.md` · `Doc-8A_Structure_Independent_Hard_Review_v0.1.md` · `Doc-8A_Structure_Patch_v0.1.1.md` · `Doc-8A_Structure_Patch_Short_ReReview_v0.1.1.md` · `Doc-8A_Structure_Freeze_Audit_v1.0.md` | Doc-8A structure authoring history (Pass → Board Hard Review [2 MAJOR + 2 MINOR + 2 NIT + 1 OBS resolved, 1 REJECTED] → Structure Patch + short re-review PASS → Structure Freeze Audit PASS) |

**Carried into Doc-8A content + Doc-8B…8G (per-suite gates):** `DR-8-HARNESS` (8B frozen first) · `DR-8-CONTRACT` (Doc-5/6/7 testability cross-check) · `DR-8-STATE` (Doc-4M drives state suites) · `DR-8-RLS` (mandatory byte-equivalence band) · `[ESC-8-TOOLING]` (test-framework ratification — Board) · `[ESC-8-API]` (additive Doc-5x patch) · `[ESC-8-CORPUS]` (corpus defect — flag-and-halt, never weaken the test) · `[ESC-8-POLICY]` (additive Doc-3 §12.2 patch) · `[ESC-8-SCOPE]`. **Per-suite oracle-readiness:** 8A/8B/8C/8E oracle-ready now (Doc-5 frozen; Doc-2/3/4M frozen); 8D/8F/8G await Doc-6/7. **Next:** Doc-8A content passes (§0–§12 + Appendix A check IDs), then Doc-8B (Test Foundation & Harness) *(frozen first per DR-8-HARNESS)*, then the discipline suites Doc-8C…8G.
