...
**Date:** 2026-06-26 (base); reconciled 2026-07-12 against post-freeze additive patches through `Doc-2_Patch_v1.0.8`/`Doc-8_SERIES_FROZEN_v1.0`
...
- **Authoritative:** `Doc-2_Domain_Model_And_Database_Blueprint_v1.0.2.md` + patches `Doc2_Patch_v1.0.2.md`, `Doc-2_Patch_v1.0.3.md`, `Doc-2_Patch_v1.0.4.md` (BuyerProfileAudit — business-action semantics only; linked pair `Doc-4C_BuyerProfileAuditToken_Patch_v1.0.2.md`), `Doc-2_Patch_v1.0.5.md` (VendorSubdomainSlug / PATCH-D2-04, ADR-024; linked set `Doc-4D_CanonicalHost_Patch_v1.0.2.md` + `Doc-6D_VendorSlugSubdomain_Patch_v1.0.1.md` + `Doc-3_Policy_Key_Registration_Patch_v1.10_VendorSubdomain.md`), `Doc-2_Patch_v1.0.6.md` (PATCH-D2-05, `identity.users.display_name`; linked pair `Doc-6C_Patch_v1.0.2.md`), `Doc-2_Patch_v1.0.7.md` (PATCH-D2-06, §5.10 suspended-at-expiry boundary; `[ESC-IDN-DELEG-EXPIRY]` cleared; companion `Doc-4M_Patch_v1.0.2.md`), `Doc-2_Patch_v1.0.8.md` (PATCH-D2-07, §7 routing-governance staff slugs; `[ESC-RFQ-SLUG]` cleared; companion `Doc-6C_Patch_v1.0.3.md`), `Doc-2_Patch_v1.0.9.md` (PATCH-D2-09, Communication support-ticket §9 audit actions for BC-COMM-4; linked pair `Doc-4H_SupportTicketAuditToken_Patch_v1.0.md`), `Doc-2_Patch_v1.0.10_VendorBuyerRelationship.md` (PATCH-D2-08, Vendor Buyer Relationship aggregate — **reconciled to `main` in the D2-08 forward-PR, renumbered from branch v1.0.9 per Scheme B**; linked set `Doc-4F_BCOPS6_Additive_Patch_v1.0.md` + `Doc-4E_VendorInvited_Payload_Additive_Patch_v1.0.md`), `Doc-2_Patch_v1.0.11_GrowthHub.md` (PATCH-D2-10, Growth Hub domain additions — **reconciled to `main` in the D2-09 forward-PR, renumbered from branch v1.0.10 per Scheme B**; one of the 10-patch Growth Hub set, see §3b), `Doc-2_Patch_v1.0.12_VendorTypePresetValues.md` (PATCH-D2-11, `vendor_type_preset` §10.3 closed six-member value domain bound by pointer to the AMD-MA-VTP-1 re-frozen rows — **reconciled to `main` in the D2-10 forward-PR, renumbered from branch v1.0.13 / PATCH-D2-10 per Scheme B + Amendment A-2**; linked pair `Doc-4D_VendorTypePreset_Pointer_Patch_v1.0.5.md`; overlay, base unedited; resolves `ESC-MKT-VENDORTYPE` values half) — all human/Board-approved, additive, carried alongside the unedited frozen base (effective v1.0.12)

**Doc-3 — RFQ Procurement Engine & Operational Specification**
- **Authoritative:** `Doc-3_RFQ_Procurement_Engine_And_Operational_Specification_v1.0.1.md` + patches `Doc-3_Patch_v1.0.2.md`, `Doc-3_Policy_Key_Registration_Patch_v1.0.md` (`core.*` keys), `Doc-3_Policy_Key_Registration_Patch_v1.1_RFQ.md` (`rfq.*` keys), `Doc-3_Policy_Key_Registration_Patch_v1.2_Marketplace.md` (`marketplace.*` keys), `Doc-3_Policy_Key_Registration_Patch_v1.3_Trust.md` (`trust.*` keys), `Doc-3_Policy_Key_Registration_Patch_v1.4_Operations.md` (`operations.*` keys), `Doc-3_Policy_Key_Registration_Patch_v1.5_Communication.md` (`communication.*` keys), `Doc-3_Policy_Key_Registration_Patch_v1.6_Billing.md` (`billing.*` keys), `Doc-3_Policy_Key_Registration_Patch_v1.7_Admin.md` (`admin.*` keys), `Doc-3_Policy_Key_Registration_Patch_v1.8_AI.md` (`ai.*` keys), `Doc-3_Policy_Key_Registration_Patch_v1.10_VendorSubdomain.md` (one `marketplace.*` key — reserved vendor-subdomain labels; ADR-024; folded 2026-07-03), `Doc-3_Policy_Key_Registration_Patch_v1.11_PublicReadRateLimit.md` (`marketplace.public_read_rate_limit`, folded 2026-07-03 with the `ESC-7-API-PRODDETAIL` E-1/E-3 pair — see 00_AUTHORITY_MAP.md; full registration list per 00_AUTHORITY_MAP.md, this index is not exhaustively kept in step) (effective v1.0.2)

**Architecture-level patches / reconfirmation**
- **Additive:** `Architecture_CD-MA-1_Patch_v1.0.md` · `Architecture_CD-MA-1_Patch_Verification_v1.0.md` · `Architecture_Freeze_Reconfirmation_v1.0.md`
- **NON-ADDITIVE (canonical base re-freeze):** `Architecture_VendorTypePreset_Amendment_v1.0.md` (**AMD-MA-VTP-1**) · `Architecture_VendorTypePreset_Amendment_Verification_v1.0.md` — re-issues the §Invariant 1 vendor-type preset register in place, five rows → six (row 1 → *Consultant*, row 4 → *Manufacturer / Workshop*, row 6 *Service Provider* added, seeding `can_service` alone); resolves P-1 + P-2; owner-ruled + independently verified, IN FORCE 2026-07-22, applied to `main` 2026-07-24. **Distinct in class from CD-MA-1** — CD-MA-1 was an additive patch carried alongside an unedited base and carries no authority for a non-additive change (amendment §1.1). Companion Doc-2 §10.3 value-domain patch + linked Doc-4D pointer patch **not yet folded on this trunk** — they land as a separate instrument. Open carries **C-6** / **OBS-2** remain OPEN.
...
- Post-freeze additive patches (human-approved, carried alongside — not yet folded into a re-issued SERIES manifest): `Doc-4B_AuditAppendRLS_Patch_v1.0.1.md` (§B10/§17.1 RLS-posture record for the in-tx audit append; realizes ADR-021; linked pair `Doc-6B_Structure_Additive_Patch_v1.0.1.md`) · `Doc-4B_OutboxAuditToken_Patch_v1.0.md` (run-level outbox dispatch/archive audit token — `entity_type = outbox_dispatch_run`/`outbox_archive_run`)
...
- Post-freeze additive patch (human-approved, linked pair with `Doc-2_Patch_v1.0.4.md`): `Doc-4C_BuyerProfileAuditToken_Patch_v1.0.2.md` (wire-level audit token realization for buyer-profile create/update — serialized `action`/`entity_type`; Doc-2 owns the business semantics, this owns the wire)
...
- Realization patches (folded, carried alongside — full detail per `00_AUTHORITY_MAP.md`): `Doc-4D_CanonicalHost_Patch_v1.0.2.md` (ADR-024, Canonical Vendor Subdomain wire realization) · `Doc-4D_PublicProductDetail_Patch_v1.0.3.md` (new `marketplace.get_public_product_detail.v1` contract, `R-ESC7-PRODDETAIL-FREEZE` E-1, folded 2026-07-03 with `Doc-5D_PublicProductDetail_Patch_v1.0.1.md` + `Doc-3_Policy_Key_Registration_Patch_v1.11_PublicReadRateLimit.md` + `ADR-025_Marketplace_Public_URL_Law.md`) · `Doc-4D_VendorSlugResolve_Patch_v1.0.4.md` (new `marketplace.resolve_vendor_slug.v1` contract, resolves `[ESC-MKT-SLUG-RESOLVE]`, folded 2026-07-11 with `Doc-5D_VendorSlugResolve_Patch_v1.0.2.md`) · `Doc-4D_VendorTypePreset_Pointer_Patch_v1.0.5.md` (PATCH-4D-VTP-01, pointer-only: adds `(Doc-2 §10.3)` to the 3 `vendor_type_preset` enum declarations; linked pair with `Doc-2_Patch_v1.0.12`, reconciled to `main` 2026-07-24 in the D2-10 forward-PR — Doc-4D label branch v1.0.4 → main v1.0.5, patch ID unchanged)
...
- **Post-freeze additive patch** (human/Board-approved 2026-07-19; reconciled to `main` in the D2-08 forward-PR): `Doc-4E_VendorInvited_Payload_Additive_Patch_v1.0.md` (PATCH-4E-VIP-01) — authoritative `VendorInvited` producer-side payload declaration adding `buyer_organization_id`; enables the BC-OPS-6 consumer; `event_version` unchanged (§16.4)
...
- **Post-freeze additive patch** (human/Board-approved 2026-07-19; reconciled to `main` in the D2-08 forward-PR): `Doc-4F_BCOPS6_Additive_Patch_v1.0.md` (PATCH-4F-BCOPS6-01) — establishes **BC-OPS-6 Vendor Buyer Relationships** (M4 contexts 5→6, aggregates 7→8; 9 authoritative contracts); realizes Doc-2 v1.0.10; linked with `Doc-4E_VendorInvited_Payload_Additive_Patch_v1.0.md`
...
- **FROZEN/authoritative:** `Doc-4M_FROZEN_v1.0.md` + additive patches `Doc-4M_Patch_v1.0.1.md`, `Doc-4M_Patch_v1.0.2.md` (effective v1.0.2); freeze `Doc-4M_Final_Freeze_Audit_v1.0.md`; structure `Doc-4M_Structure_FROZEN_v1.0.md`
...
- Post-freeze editorial index corrections (human-approved, additive — no state/transition/ownership change): `Doc-4M_Patch_v1.0.1.md` (M5 matrix — Organization + Membership rows corrected to match Doc-2 §5.1/§5.2) · `Doc-4M_Patch_v1.0.2.md` (M5 matrix — Delegation Grant rows corrected to match Doc-2 §5.10 as patched by `Doc-2_Patch_v1.0.7`)

---

## 3b. Growth Hub P0 Additive Patch Set (10 linked patches — FOLDED 2026-07-19, Board resolution)

Atomic set (Invite-to-Register Growth Hub; authority `governanceReviews/GrowthHub_P0_Additive_Patch_Set_PROPOSAL_v1.0.md` content v1.4; per-patch Review-A→Review-B→fix-verification each 0·0·0; Final-Gate Set Integrity Audit 7 scopes: BLOCKER 0 · MAJOR 0 · MINOR 0 · Dangling 0). **Reconciled to `main` 2026-07-23 (D2-09 forward-PR), documentary set only** — Doc-2 → v1.0.11/PATCH-D2-10 · Doc-3 → v1.12 · Doc-7E → v1.0.2 (Scheme B + Amendment A-1). Growth Hub backend code, Prisma schema/migrations and tests are **not** carried (gated by `ESC-IDN-INVITE-ACCOUNT` + the unresolved M0 `core.write_outbox_event.v1` divergence).

| Corpus copy | Extends |
|------|------|
| `Doc-2_Patch_v1.0.11_GrowthHub.md` (PATCH-D2-10) | Doc-2 → effective v1.0.11 (2 M1 entities · machines 5.11/5.12 · slug 46→47 · M1's first 2 §8 events · 2 §9 actions) |
| `Doc-3_Policy_Key_Registration_Patch_v1.12_GrowthHub.md` | Doc-3 §12.2 (7 `identity.*` + 2 `billing.*` keys; Q-6/Q-12 values Board-pending) |
...
| `Doc-6B_Structure_Additive_Patch_v1.0.1.md` | **Post-freeze additive patch** (human-approved 2026-06-30, carried alongside — not yet folded into a re-issued SERIES manifest): append-only context-bound `FOR INSERT` RLS policy realizing `core.audit_records` append-admission (ADR-021; linked pair `Doc-4B_AuditAppendRLS_Patch_v1.0.1.md`); **realization (D4) not yet performed** — bound by a normative Deployment Constraint |
...
| `Doc-6C_Patch_v1.0.1.md` · `Doc-6C_Patch_v1.0.2.md` · `Doc-6C_Patch_v1.0.3.md` | **Post-freeze additive patches** (human/Board-approved; effective v1.0.3): v1.0.1 permission-slug count correction (editorial) · v1.0.2 `identity.users.display_name` DDL realization (linked pair `Doc-2_Patch_v1.0.6.md`) · v1.0.3 permission-slug count overlay 43→45 (editorial, realizes the two staff slugs added by `Doc-2_Patch_v1.0.8.md`) |
...
| `Doc-6D_VendorSlugSubdomain_Patch_v1.0.1.md` | **Post-freeze additive patch** (human/Board-approved 2026-07-03, ADR-024): DDL realization of the canonical vendor-subdomain slug law; linked set with `Doc-2_Patch_v1.0.5.md` (business semantics) + `Doc-4D_CanonicalHost_Patch_v1.0.2.md` + `Doc-3_Policy_Key_Registration_Patch_v1.10_VendorSubdomain.md` |
...
## 5d. Doc-8 Program (Test & Conformance Realization — **COMPLETE / FROZEN**, 8A–8G)

> The verification sibling of the Doc-5 API / Doc-6 DB / Doc-7 FE programs. **Program FROZEN 2026-06-26** — `Doc-8_SERIES_FROZEN_v1.0` (program closure manifest, all 7 deliverables). **Doc-8A is the metastandard** (the Doc-5A/6A/7A analog); it governs the test-discipline-partitioned realizations Doc-8B…8G via Appendix A (`CHK-8-xxx`). Realizes the full frozen *what*-corpus (Doc-2/3/4 + 12 invariants/5 firewalled signals/moat — the test oracle) + the Doc-5/6/7 realization contracts (under test) as deterministic, isolated, CI-gating conformance suites. **Doc-8 is the conformance harness yet subordinate to its oracle** (a downward gate over implementations; upward-subordinate to the corpus — governance §3/§8 rule 5): a red test = code defect (fix it) or corpus defect (`[ESC-8-CORPUS]`, flag-and-halt) — never weaken the assertion. **Authored-not-run throughout:** every suite's design/coverage is FROZEN against the frozen oracles; PASS/FAIL is recorded at execution once the implementation Code exists.

| File | Purpose |
|------|---------|
| `Doc-8_SERIES_FROZEN_v1.0.md` | **Program closure manifest** — all 7 deliverables (8A metastandard + 8B harness + 8C…8G discipline suites) FROZEN; the closed cross-suite conformance-fabric allocation (assert-once/compose-elsewhere); standing program properties (authored-not-run, merge gate, never-weaken/never-coin). Points; the per-doc manifests below are authoritative |
| `Doc-8A_SERIES_FROZEN_v1.0.md` | **Doc-8A FROZEN** — Test & Conformance Realization Metastandard; effective = `Doc-8A_Structure_v1.0_FROZEN` + `Doc-8A_Content_v1.0_Pass1…3` (+ per-pass patches) + freeze audits. Fixes R1–R12; **test-discipline partition** (Doc-8B Foundation/Harness *(frozen first)* + 8C Contract/API + 8D Persistence/Migration/RLS + 8E Domain/Invariant/State-Machine + 8F Integration/Event-Flow + 8G Frontend/E2E *(frozen last)*); cross-cutting conformance-concern allocation (RLS/non-disclosure→8D, firewall/invariant/state→8E — assert once, compose elsewhere); Appendix A = **39 `CHK-8-001…081` checks / 9 bands (A–I)** (per-suite freeze gate); inherits the RLS positive/negative/cross-tenant byte-equivalence gate (`Doc-6A R8/§4`) + the migration gate (`Doc-6A §11`); `ERR-8A-1` folded in (structure ID-anchor "Doc-6A §7"→"§3+Doc-4B"). Coins nothing |
...
**Doc-8D (Persistence, Migration & RLS Conformance Suite) — FROZEN 2026-06-26** (`Doc-8D_SERIES_FROZEN_v1.0`): the **defining suite** for immutability (#8, `CHK-8-022`) and RLS positive/negative/cross-tenant byte-equivalence (#11, `CHK-8-024`); consumes Doc-8B by pointer. **Schema-inventory-driven (D1)** — coverage ≡ the frozen Doc-6 DDL (dynamic as 6E…6K froze); Band C also fixes schema-constraint, multi-currency, migration (forward-only/expand-contract), cross-module integrity (no cross-schema FK). `CLAR-8D-1` folded in: **#11 byte-equivalence proper = the buyer-private case** (`buyer_vendor_statuses`, M4/Doc-6F), **distinct from** general marketplace-visibility RLS (Doc-6D, ready now). Structure + 2 content passes each Hard-Reviewed + patched + closure-checked (0 BLOCKER/MAJOR/MINOR) + Freeze Audits PASS. Coins nothing.

**Doc-8E (Domain, Invariant, Firewall & State-Machine Conformance Suite) — FROZEN 2026-06-26** (`Doc-8E_SERIES_FROZEN_v1.0`): the **defining suite** for the 5-signal firewall, the 12 Core Invariants, the moat, and the Doc-4M state machines; consumes Doc-8B by pointer. **Two firewall assertion shapes** (`CHK-8-030/031`): non-cross-mutation (vary A, assert B independent) + non-dominance (no single signal unilaterally decides, per the frozen scoring rule — never an invented metric). One suite per invariant (`CHK-8-032`; **exception:** #8/#11 are Doc-8D-defined, 8E references). Moat as governed system property (`CHK-8-033`). Doc-4M per-machine legal/illegal transitions + edge coverage (`CHK-8-040…042`; optimistic-UI convergence defined here, executed in 8G). Exports canonical assert-once helpers composers invoke, never re-implement. Structure + 2 content passes each Hard-Reviewed + patched + closure-checked (0 BLOCKER/MAJOR/MINOR) + Freeze Audits PASS. Coins nothing.

**Doc-8F (Integration & Event-Flow Conformance Suite) — FROZEN 2026-06-26** (`Doc-8F_SERIES_FROZEN_v1.0`): consumes the Doc-8B outbox observer/drainer + mocked Inngest by pointer. **Catalog-inventory-driven (F1)** — event inventory ≡ the frozen Doc-4J catalog + Doc-4L flow (zero events coined; a Doc-2 §8 ↔ Doc-4J divergence is `[ESC-8-CORPUS]`). Band F: cross-module boundary (static import-graph, `contracts/`-only) · write-plus-emit atomicity (business write + declared event set commit/rollback together) · payload/dispatch/fan-out (`pending→dispatched`, forward-only) · consumer-effect locality. Composes firewall-via-events (8E) + non-disclosure-via-events (8D §5.4, buyer-private/Doc-6F-deferred). Structure + 2 content passes each Hard-Reviewed + patched + closure-checked (0 BLOCKER/MAJOR/MINOR) + Freeze Audits PASS. Coins nothing, coins zero events.

**Doc-8G (Frontend & E2E Conformance Suite) — FROZEN 2026-06-26** (`Doc-8G_SERIES_FROZEN_v1.0`) — **the final Doc-8 deliverable (7 of 7), closing the program**: consumes Doc-8B (Playwright/axe/snapshots) by pointer. **Surface-inventory-driven (G1)** — ≡ the complete frozen Doc-7 set (7B kit + 7C shell + 7D…7H surfaces; none coined). Band G: component conformance (composes 8E's #9 Content≠Presentation) · a11y (WCAG-AA/axe) · visual-regression · e2e (invokes only frozen Doc-5 contracts, 8C's oracle, not re-run) · currency · **UI non-disclosure (`CHK-8-065`, `Doc-7 R8`, load-bearing):** (a) Vendor-view byte-equivalence — a blacklisted vendor's workspace ≡ a non-matched vendor's (composes Doc-8D §5.4; data: Doc-6F) — and (b) buyer-private CRM confinement (never leaks to a vendor-facing surface). Structure + 2 content passes each Hard-Reviewed + patched (1 MAJOR each closed) + closure-checked (0 open BLOCKER/MAJOR/MINOR) + Freeze Audits PASS. Coins nothing, coins no screen.

**Program-level carried (named channels, surfaced only at execution):** `DR-8-HARNESS`/`DR-8-CONTRACT`/`DR-8-STATE`/`DR-8-RLS` all **satisfied** (8B/8C/8E/8D respectively) · `[ESC-8-TOOLING]` **RESOLVED** (Doc-8B D1) · `ERR-8A-1` + `CLAR-8D-1` folded in · `[ESC-8-CORPUS]` (corpus defect — flag-and-halt, never weaken/never coin) · `[ESC-8-API]` (additive Doc-5x patch) · `[ESC-8-POLICY]` (additive Doc-3 §12.2 patch) · `[ESC-8-SCOPE]`. **Execution-deferred facets** (authored, pointer-bound, none dropped): the buyer-private #11 byte-equivalence data scenario (Doc-6F) · event emitters/consumers + import-graph lint (Code) · migration/codegen (Code) · the whole 8G suite (UI Code). **Doc-8 = COMPLETE (7/7 frozen); sibling programs Doc-5/6/7 already complete. Next: implementation Code — gated by the full Doc-8 conformance fabric + AI Coding Supervisor/human review (CLAUDE.md §8).**
...
| `Wave0_Baseline_Report_v1.0.md` · `Wave1_Baseline_Report_v1.0.md` · `Wave2_Baseline_Report_v1.0.md` | Per-wave delivery baseline reports (CI evidence, merge commits, deploy status) — Wave 0/1/2, all DELIVERED to `main` |
