...
**Carried into Doc-8C…8G (per-suite gates):** `DR-8-HARNESS` **satisfied** (Doc-8B; consumed by pointer) · `DR-8-CONTRACT` (Doc-5/6/7 testability cross-check) · `DR-8-STATE` (Doc-4M drives state suites) · `DR-8-RLS` (mandatory byte-equivalence band) · `[ESC-8-TOOLING]` **RESOLVED** (Doc-8B D1) · `[ESC-8-API]` (additive Doc-5x patch) · `[ESC-8-CORPUS]` (corpus defect — flag-and-halt, never weaken the test) · `[ESC-8-POLICY]` (additive Doc-3 §12.2 patch) · `[ESC-8-SCOPE]`. **Per-suite oracle-readiness:** 8C/8E oracle-ready now (Doc-5 frozen; Doc-2/3/4M frozen); 8D growing (Doc-6B+6C frozen — `core` + `identity` testable); 8F/8G await Doc-6/7. **Next:** discipline suites Doc-8C…8G consume the frozen harness.
