# Doc-3 — POLICY Key Registration Patch v1.11 (Module 2 Public-Read Rate Limit)

> **✅ STATUS: APPROVED (human — owner/Board ruling 2026-07-03) + FOLDED into the corpus.**
> This is the corpus copy `generatedDocs/Doc-3_Policy_Key_Registration_Patch_v1.11_PublicReadRateLimit.md`,
> registered in `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`, carried **alongside** the unedited frozen
> Doc-3 — **no frozen file edited in place.**
> Origin/provenance: `governanceReviews/Doc-3_Policy_Key_Registration_Patch_v1.11_PublicReadRateLimit_PROPOSAL.md`.
> Folded together with the linked E-1 pair (`Doc-4D_PublicProductDetail_Patch_v1.0.3` +
> `Doc-5D_PublicProductDetail_Patch_v1.0.1`) and E-3 (`ADR-025_Marketplace_Public_URL_Law`) — the full
> Annex E instrument set of `R-ESC7-PRODDETAIL-FREEZE` landed together at this fold.
...
| Patch Authority | `ESC-7-API-PRODDETAIL` Board Resolution **`R-ESC7-PRODDETAIL-FREEZE`** (owner Board, 2026-07-03), Annex E Decision #6 — authorized this instrument. |
| Patch Type | **Additive registration only** — registers one `marketplace.*` POLICY key referenced by the now-folded Doc-4D v1.0.3 / Doc-5D v1.0.1 patches. No validation-semantic, routing, trust, procurement, ownership, enforcement-mechanism, or algorithm content. No existing key modified or removed. |
| Sole Purpose | Satisfy **Doc-4A §18.2** ("a POLICY key MUST exist in Doc-3 §12.2") for the identifier registered here, unblocking the E-1 contract-freeze gate of `Doc-4D_PublicProductDetail_Patch_v1.0.3.md` and `Doc-5D_PublicProductDetail_Patch_v1.0.1.md` §Rate-Limiting. |
| Conforms To | Master_System_Architecture_v1.0_FINAL.md, ADR_Compendium_v1.md, Doc-2 v1.0.5, Doc-3 v1.0.2, Doc-4A v1.0 (FROZEN), Doc-4D v1.0.3 (folded together), Doc-5D v1.0.1 (folded together) |
| Linked Document | `Doc-4D_PublicProductDetail_Patch_v1.0.3.md`; `Doc-5D_PublicProductDetail_Patch_v1.0.1.md`; `governanceReviews/ESC-7-API-PRODDETAIL_Product_Detail_Architecture_Plan_v1.0_PROPOSAL.md` (the resolution this instrument realizes). Precedent: `Doc-3_Policy_Key_Registration_Patch_v1.2_Marketplace.md` and `Doc-3_Policy_Key_Registration_Patch_v1.10_VendorSubdomain.md` (both extend the same `marketplace.*` domain row). |
| Status | **APPROVED — FOLDED 2026-07-03** (human owner ruling). |
...
throughput value as a literal — the identifier is registered here.
...
change. Status: APPROVED — FOLDED 2026-07-03.*
