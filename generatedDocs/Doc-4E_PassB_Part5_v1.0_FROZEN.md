# Doc-4E — RFQ Procurement Engine — Pass-B (Hardening) Part 5 v1.0 (FROZEN) — BC-5 Evaluation/Comparison + BC-6 Award/Closure

## BC-5 Buyer Evaluation & Comparison + BC-6 Procurement Decision & Closure Hardening (§E8)

| Field | Value |
|---|---|
| Document | Doc-4E — **Pass-B Part 5 v1.0 (FROZEN)** — final immutable Part-5 baseline — Module 3 RFQ Procurement Engine (`rfq` schema) |
...
| Status | **Part-5 FROZEN — final immutable baseline.** Consolidates `Doc-4E_Content_v1.0_PassB_Part5_EvaluationComparisonAward.md` as amended by `Doc-4E_PassB_Part5_Patch_v1.0.md`; certified by `Doc-4E_PassB_Part5_Freeze_Audit_v1.0.md`. **Completes Pass-B (Parts 1–5).** |
...
**8. Event Binding** — Emitted **none** (consumes quotation events; emits nothing) · Consumed **`QuotationSubmitted`** / **`QuotationWithdrawn`** (Doc-2 §8). **`quotation_revised` is NOT a Doc-2 §8 event:** quotation revision has no domain event (Part 4 §E7.2 — `submitted → submitted` new-version transition + `edit (new version)` audit only); it is an **internal application-layer trigger** — when `rfq.revise_quotation.v1` commits a new version, the application layer **synchronously invokes** comparison regeneration in-process (no outbox, no domain event, no CDC, no separate contract). · Idempotency: idempotent consumer/invocation (Doc-4A §16) — replay re-generates the same version content, no duplicate.
...
**12. AI-Agent Implementation Notes** — **Never auto-recommend or mark a winner** (Doc-3 §9.1 FIXED) — the statement summarizes; humans decide. Bind vendor-standing columns to `matching_results` (PA-04), **not** a live Trust/Marketplace call. Buyer-private columns (own status/notes) are buyer-only (non-disclosure). Re-version on every quotation event; idempotent. **`quotation_revised` is an internal trigger, not an event:** do **not** coin a Doc-2 §8 event/outbox/contract — quotation revision is a non-event (Part 4); regeneration is invoked synchronously in-process by `rfq.revise_quotation.v1` on version commit.
...
| delegation (buyer decision) | 5 DELEGATION | Doc-4A §6B; Doc-3 §9.4; H.3 | **not delegation-eligible** — buyer decision authority is the buyer org's own membership; no §6B grant path | `AUTHORIZATION` (if a grant presented) |
...
| delegation (buyer decision) | 5 DELEGATION | Doc-4A §6B; Doc-3 §9.4; H.3 | **not delegation-eligible** — clarification/best-and-final is the buyer org's own decision authority; no §6B grant path | `AUTHORIZATION` (if a grant presented) |
...
| delegation (buyer decision) | 5 DELEGATION | Doc-4A §6B; Doc-3 §9.4; H.3 | **not delegation-eligible** — award authority is the buyer org's own membership (`can_award_rfq`); no §6B grant path | `AUTHORIZATION` (if a grant presented) |
...
**12. AI-Agent Implementation Notes** — **Single award only** (Doc-2 §5.4 cardinality FIXED) — exactly one `selected_quotation_id`; split needs are a **re-issue** (BC-1, Part 1 `reissue_rfq`), never multi-award. **Award of a non-shortlisted quotation is foreclosed:** `selected_quotation_id` must be in the persisted shortlist set (`shortlisted_quotation_ids`, §E8.2); the **shortlist-membership check executes before award commit** (same atomic transaction). The **engagement is Operations'** (DE-4), created off `RFQClosedWon` — never authored here. RFQ + quotation transitions are **one atomic transaction** (Doc-2 §10.11.4). Award records deal value. Terminal — never reopen. No plan influence on award (§4B).
...
| RFQ owned by active org | 4 SCOPE | Doc-4A §7.3 | buyer controlling org owns RFQ | `NOT_FOUND` (collapse, §7.5) |
| delegation (buyer decision) | 5 DELEGATION | Doc-4A §6B; Doc-3 §9.4; H.3 | **not delegation-eligible** — close-without-award authority is the buyer org's own membership; no §6B grant path | `AUTHORIZATION` (if a grant presented) |
...
**6. State Machine Enforcement** — **The first buyer open drives `quotations_received → buyer_reviewing`** (Doc-2 §5.4); the system also auto-advances at quotation-window close — **whichever is first** (Doc-3 §1.2; Pass-A PA-16). **Mechanism:** on a buyer open, **if** RFQ state = `quotations_received`, advance to `buyer_reviewing` **within the same transaction used by the comparison read** (read + conditional state write committed atomically — Doc-2 §10.11.4); **subsequent reads perform no transition** (state-guarded). **No new event, outbox message, background job, CDC, or additional contract** — synchronous in-transaction side effect of this read only. Vendor-facing status shows "under evaluation" only from `buyer_reviewing`; the first-open transition is audited via the lifecycle. **No edge invented** (existing Doc-2 §5.4 edge).
...
**12. AI-Agent Implementation Notes** — The comparison shows standing bands but **never an auto-recommended winner** (Doc-3 §9.1 FIXED). **Buyer-private columns are never exposed to vendors** (non-disclosure). The first buyer open (or window-close, whichever first) advances `quotations_received → buyer_reviewing` — the only side effect of this read (PA-16). **Mechanism:** inline, same transaction as the read, guarded by `if state = quotations_received`; subsequent reads no-op (idempotent on state); no event/outbox/job/CDC/contract. Display data is from `matching_results` (PA-04), never a live signal read.
