...
> Corpus copy `generatedDocs/Doc-3_Policy_Key_Registration_Patch_v1.14_GrowthHub.md`, registered in `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`,
> carried **alongside** its unedited frozen base document(s) — **no frozen file edited in place.**
> Origin/provenance: `governanceReviews/Doc-3_Policy_Key_Registration_Patch_v1.14_GrowthHub_PROPOSAL.md` (the authoring + per-patch
...
# Doc-3 — POLICY Key Registration Patch v1.14 (Growth Hub: `identity.*` block extension + `billing.*` extension)

| Field | Value |
|---|---|
| **Status** | **PROPOSED** — gated on per-patch Review-A → Review-B → Board fold (atomic with the 10-patch Growth Hub set). Additive `Doc-3 §12.2` registration; patches frozen Doc-3 **without editing it in place** (the v1.0–v1.12 channel). |
| **Date** | 2026-07-19 |
| **Type** | **Additive POLICY-key registration** (the same §12.2 channel as `core.*` v1.0 and `rfq.*`…`ai.*` v1.1–v1.8, `identity.*` v1.9, and the v1.10–v1.12 singles). **No semantic / governance-signal / state / event / contract change.** |
| **Version note** | The stamped packet (§A.5/§B2) named this patch `…v1.13_GrowthHub_Identity`. **v1.13 is claimed** by the pending `Doc-3_Policy_Key_Registration_Patch_v1.13_EvidenceHandling_PROPOSAL.md` (its own header: "v1.13 is the next free number; re-verify at fold") — the folded chain ends at **v1.12**, so this patch takes **v1.14** (the Doc-4H patch's §6 collision fold-note, Review-B F-5). If EvidenceHandling is renumbered or rejected before the atomic fold, the Board re-verifies both numbers at fold; the sibling patches cite this patch by the resolved name. The packet's `_Identity` filename suffix is dropped — this file registers **both** namespace blocks (the packet's "+ a `billing.*` extension"), so the suffix would misdescribe the scope. **Same collision class, Doc-2 axis (Review-B F-1):** the Growth Doc-2 sibling is likewise renumbered **v1.0.10 / PATCH-D2-09** (the parallel train folded Vendor-Buyer-Relationship as `Doc-2_Patch_v1.0.9.md` / PATCH-D2-08, `573a349`) — both axes join the Board's re-verify-at-fold list. |
...
The Growth Hub M1 contracts (`Doc-4C_…v1.0.3` §C13) and the invitation entities (`Doc-2_Patch_v1.0.10`
...
- **No open-invitation (link/qr) redemption-cap key** — `max_redemptions` is a per-row column
  (`Doc-2_Patch_v1.0.10` §1), set at issue; a platform default for it, if wanted, is a follow-up patch.
...
PROPOSED — atomic fold with the 10-patch Growth Hub set. v1.13→v1.14 renumber per the collision fold-note.
