# Doc-4J — Admin Operations — API & Integration Contracts — Structure Proposal v0.1

| Field | Value |
|---|---|
| Document | Doc-4J — **Structure Proposal v0.1** — canonical Table of Contents proposal for Module 8 — Admin Operations (`admin` schema, `admin_` namespace) — the **platform-governance & operations layer** |
| Status | **Structure Proposal — pre-freeze.** Defines the complete Module-8 structure (bounded contexts, aggregates, events, dependencies, maps) before contract authoring. **Not Pass-A; not Pass-B.** Next stage: Independent Hard Review → Structure Patch → Structure FROZEN → Pass-A. |
| Module | Module 8 — Admin Operations (`admin` schema) — platform-staff governance/operations: moderation, ban enforcement, category/vendor suggestion decisions, data import, verification workflow, vendor outreach |
| Authority | Doc-4_Governance_Note_v1.0.md; Doc-4A v1.0 (FROZEN) governs this document |
| Conforms To | Master_System_Architecture_v1.0_FINAL.md, ADR_Compendium_v1.md, Doc-2 v1.0.3, Doc-3 v1.0.2, Doc-4A v1.0, Doc-4B v1.0, Doc-4C v1.0, Doc-4D v1.0, Doc-4E v1.0, Doc-4F v1.0, Doc-4G v1.0, Doc-4H v1.0, Doc-4I v1.0 — all FROZEN |
| Family-map basis | Doc-4A §1.3: **Doc-4J = Admin Operations (Module 8)**; Appendix B namespace `admin_` (admin) |
...
| Audience | Doc-4J content-pass authors; Claude Code / Cursor / OpenAI Codex / backend / frontend / QA / AI coding agents |
...
*End of Doc-4J — Admin Operations — Structure Proposal v0.1. Structure only — no contract, command, query, payload, validation matrix, state-machine detail, or audit action instantiated. Module 8 (`admin` schema, `admin_` namespace) decomposes into 6 bounded contexts (BC-ADM-1…6) owning 6 aggregates (Doc-2 §2 — Moderation Case, Ban Action, Suggestion, Import Job, Verification Task, Outreach Campaign), each in exactly one context; the Suggestion aggregate's three roots co-locate in BC-ADM-3. Cross-module dependencies DR-ADM-1/MKT/RFQ/OPS/TRUST/PC explicit; produced events `VendorBanned` (BC-ADM-2, single-authorship, Doc-2 §8); consumed events the Doc-2 §8 governance-surfacing events (e.g., RFQ→moderation) bound at content authoring; escalation markers `[ESC-ADM-SLUG]`/`[ESC-ADM-AUDIT]`/`[ESC-ADM-POLICY]`/`[ESC-ADM-EVENT]` carried, plus carried markers DC-3/DD-3/DD-4/DF-5 resolved as the owning module. **Family-map reconciliation recorded:** the brief titled this "Module 8 — Event Contracts," but the FROZEN Doc-4A §1.3 family map fixes Doc-4J = Admin Operations and assigns cross-module event-flow to the non-normative Doc-4L; per the user's confirmation this document is authored as Doc-4J = Admin Operations (events stay owned per producing module under single-authorship; no event re-ownership introduced). Bound by pointer to Architecture, ADRs, Doc-2 v1.0.3, Doc-3 v1.0.2, Doc-4A/4B/4C/4D/4E/4F/4G/4H/4I v1.0 (FROZEN); the procurement moat (no matching/routing/ranking/supplier-selection/award decision), the trust firewall (no score — Trust stores, Admin decides workflow), and non-disclosure (link content never vendor-visible) are preserved; "Admin decides; the owning module stores"; business authority for procurement/scoring remains with the originating modules; nothing invented. Next: Independent Hard Review.*
