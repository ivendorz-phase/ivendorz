...
| 7D | Public Surface (anonymous) | **STRUCTURE FROZEN 2026-06-26** (`Doc-7D_Structure_v1.0_FROZEN`) — PR1–PR10; first **surface** doc; anonymous Public-projection reads only (verified core BC-MKT-6 `search_catalog`/`list_vendor_directory`/`get_public_vendor_profile`; microsite/catalog/ads bind-or-`[ESC-7-API]`); no active-org, no anonymous mutation (CTAs→7E auth-entry); microsite = M2 content + vendor-branded presentation; no buyer-private concept (Inv #11). Hard Review (0 MAJOR + 3 MINOR + 1 NIT) + Structure Freeze Audit PASS. **Content passes NEXT** |
