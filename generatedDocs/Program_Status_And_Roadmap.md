...
| 7E | Account & Identity Shell | **STRUCTURE FROZEN 2026-06-26** (`Doc-7E_Structure_v1.0_FROZEN`) — ER1–ER11; two areas `(auth)` (login/signup/recovery, Supabase Auth) + `(app)` (mgmt screens). Binds Doc-5C §C4–§C7/§C9/§C10/§C11 (user/org/membership/role/delegation/settings) + Doc-5I BC-BILL-1…6 (subscription/usage/lead/platform-invoice/reward). Seam: **§C8 switcher = Doc-7C** (not 7E). Firewalls: platform-invoice≠trade-invoice (R6), entitlement out-of-wire (R10), delegated-access check server-side (R5). User-action commands vs System-issued reads (R11). Carried `[ESC-IDN-DELEG-EXPIRY]` (reinstate not finalized) + `[ESC-7-API-SIGNUP]`. Hard Review (0 MAJOR + 3 MINOR + 1 NIT) + Structure Freeze Audit PASS. **Content passes NEXT** |
...
**Doc-8 Test & Conformance program — STARTED.** `Doc-8A` (metastandard) + `Doc-8B` (Foundation/Harness) FROZEN 2026-06-26 (`[ESC-8-TOOLING]` RESOLVED: Vitest + Playwright + TS-native SQL). **Next: discipline suites Doc-8C…8G** (consume the frozen harness).
