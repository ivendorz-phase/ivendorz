...
| 6G…6K | M5…M9 schemas | NOT STARTED |

**Carried into Doc-6G…6K (per-module gates):** `DR-6-CORE/API/STATE` · `[ESC-6-SCHEMA]` · `[ESC-6-POLICY]` (additive Doc-3 §12.2 patch per module — `core`/`rfq`/`marketplace`/`operations` already registered) · `[ESC-6-API]`.
...
**Doc-6 Database program — STARTED.** `Doc-6A` (metastandard) + `Doc-6B` (M0 `core`) + `Doc-6C` (M1 `identity`) + `Doc-6D` (M2 `marketplace`) + `Doc-6E` (M3 `rfq` — the moat) + `Doc-6F` (M4 `operations` — the blacklist owning side + money boundary) FROZEN 2026-06-26. **6 of 11 Doc-6 deliverables frozen. Next: Doc-6G (M5 `trust`).**
...
Next:
1. **Doc-6G (M5 `trust`)** — next per-module schema; the **governance-signal owner** (Trust Score / Performance Score / Verification / Financial Tier — the platform-wide signals M2 reflects and M4 never mutates); the firewall's **authoritative side**; scores **auto-calculated under the System actor, never hand-edited**; verification records + fraud signals + public reviews. The Doc-6F blacklist + the Doc-6D bands now have their owning source.
2. **Doc-6H…6K** — remaining module schemas (M6…M9), dependency order, each gated by Doc-6A Appendix A. (Doc-8 #11 byte-equivalence target `Doc-6F` now FROZEN — the deferred 8D/8F buyer-private facets are unblocked for execution.)
