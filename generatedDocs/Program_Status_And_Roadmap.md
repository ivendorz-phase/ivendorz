...
| 6F…6K | M4…M9 schemas | NOT STARTED |

**Carried into Doc-6F…6K (per-module gates):** `DR-6-CORE/API/STATE` · `[ESC-6-SCHEMA]` · `[ESC-6-POLICY]` (additive Doc-3 §12.2 patch per module — `core`/`rfq`/`marketplace` already registered) · `[ESC-6-API]`.
...
**Doc-6 Database program — STARTED.** `Doc-6A` (metastandard) + `Doc-6B` (M0 `core`) + `Doc-6C` (M1 `identity`) + `Doc-6D` (M2 `marketplace`) + `Doc-6E` (M3 `rfq` — the moat) FROZEN 2026-06-26. **Next: Doc-6F (M4 `operations`).**
...
Next:
1. **Doc-6F (M4 `operations`)** — next per-module schema; post-award docs (LOI/PO/challan/invoice/payment/WCC) + finance records + **the private Vendor CRM** (`buyer_vendor_statuses` — the blacklist's owning side, **non-disclosure invariant**, served to M3 routing via CRM service only) + two-sided engagement (both parties via party columns + RLS) + the **money-record boundary** (records only; no funds custody).
2. **Doc-6G…6K** — remaining module schemas (M5…M9), dependency order, each gated by Doc-6A Appendix A.
