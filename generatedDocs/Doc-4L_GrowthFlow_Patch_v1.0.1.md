...
> Corpus copy `generatedDocs/Doc-4L_GrowthFlow_Patch_v1.0.1.md`, registered in `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`,
> carried **alongside** its unedited frozen base document(s) — **no frozen file edited in place.**
> Origin/provenance: `governanceReviews/Doc-4L_GrowthFlow_Patch_v1.0.1_PROPOSAL.md` (the authoring + per-patch
...
| **Authority** | Growth Hub Architecture (**FROZEN** 2026-07-19) §A.3/§A.4 + the stamped **manifest row (:57)** ("2 flow rows + L3 permission row") and **§B5** ("Flow rows: … L3 permission row: …") — both direct this content to Doc-4L; Doc-2 §7 as amended by `Doc-2_Patch_v1.0.10` §3 (the slug); Doc-2 §8 as amended by `Doc-2_Patch_v1.0.10` §4 (the events); Doc-4A §0.3 (reference-never-restate). |
| **Depends on** | `Doc-2_Patch_v1.0.10` (slug + events), `Doc-4C_…v1.0.3` (producing contracts), `Doc-4I_…` (M7 consumer branch), `Doc-4H_…` (M6 consumer), `Doc-4J_…v1.0.1` (catalog registration). **Atomic fold** — these rows dangle without them. |
...
`Doc-2_Patch_v1.0.10` §3 — unbundled default, O/D/M), never here:

| Permission | Owner Module | Owner BC | Reference Source |
|---|---|---|---|
| `can_manage_growth_invites` | 1 — Identity | Growth invitations (`growth_invitations` / `invitation_conversions`; contract group §C13) | Doc-2 §7 (as amended by `Doc-2_Patch_v1.0.10`); Doc-4C (FROZEN) + `Doc-4C_…v1.0.3` |
...
- **OBS-1** — L3's staff table froze at the 7-slug Doc-2 v1.0.3 set; `Doc-2_Patch_v1.0.8` later added 2
  routing-governance staff slugs (effective staff = 9; total catalog **46** under the effective v1.0.9
  baseline — the folded Vendor-Buyer-Relationship patch added one tenant slug — and **47** with the
  growth slug, matching `Doc-2_Patch_v1.0.10` §3). Per
...
- **OBS-2** — the frozen end-note says "35 tenant … slugs" while the frozen L3 tenant table itself carries
  ...
  tenant slugs … stand unchanged"; the folded `Doc-2_Patch_v1.0.9` VBR patch then took tenant 36→37).
...
role/actor/business rule/state transition created (the slug and events are created by `Doc-2_Patch_v1.0.10`,
