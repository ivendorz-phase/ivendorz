...
> Corpus copy `generatedDocs/Doc-4H_GrowthDelivery_Patch_v1.0.1.md`, registered in `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`,
> carried **alongside** its unedited frozen base document(s) — **no frozen file edited in place.**
> Origin/provenance: `governanceReviews/Doc-4H_GrowthDelivery_Patch_v1.0.1_PROPOSAL.md` (the authoring + per-patch
...
| **Depends on** | `Doc-2_Patch_v1.0.10` §4 (the event), `Doc-4C_…v1.0.3` (the delivery-payload contract — internal-service, M6 sole caller), `Doc-4J_…v1.0.1` (catalog), `Doc-4L_…v1.0.1` (flow row L9-1), `Doc-3_…v1.14_GrowthHub` (the invite POLICY keys — renumbered v1.13→v1.14, §6 fold-note). **Atomic fold** — the consumer's trigger and its payload source do not exist without them. |
...
**2. Request Schema** *(driven by the consumed `InvitationIssued` event — not a public API surface; fields = the event payload, Doc-2 §8 as amended by `Doc-2_Patch_v1.0.10` §4)*
...
- Carried markers **identity-unchanged**: DH-1/DH-8, `[ESC-COMM-AUDIT]`, `[ESC-COMM-POLICY]`,
  ...
  unchanged. Carried realization: event/payload = `Doc-2_Patch_v1.0.10` §4; payload contract =
  `Doc-4C_…v1.0.3` §C13; catalog = `Doc-4J_…v1.0.1`; flow = `Doc-4L_…v1.0.1` L9-1; POLICY keys =
  `Doc-3_…v1.14_GrowthHub` (`[DC-5]`); consumer wiring (Inngest) = implementation scope.
  ...
  **contested** by `Doc-3_Policy_Key_Registration_Patch_v1.13_EvidenceHandling_PROPOSAL.md` (Review-B
  F-5) — **resolved: the Phase-3 sibling is authored as `…v1.14_GrowthHub`**; the Board re-verifies both
  numbers at fold.
