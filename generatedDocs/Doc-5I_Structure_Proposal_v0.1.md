# Doc-5I — Monetization / Billing (M7 `billing`) API Realization — Structure Proposal v0.1

| Field | Value |
|---|---|
| Document | Doc-5I — Monetization / Billing (Module 7) — API Realization — Structure Proposal |
| Status | **PROPOSAL v0.2** — Board Review applied (0 BLOCKER · 2 MAJOR · 3 MINOR · 3 NITPICK resolved per `Doc-5I_Structure_Independent_Hard_Review_v1.0.md` + `Doc-5I_Structure_Patch_v1.0.md`); ready for Structure Freeze Audit |
...
| BC-BILL-2: `purchase_subscription`, `cancel_subscription` | User command (21.4; subscription machine R7; SubscriptionPurchased/Expired events R9) | **§5** `POST` |
...
| BC-BILL-2: `renew_subscription`, `expire_subscription` | System period-end jobs (21.5; R7; emit SubscriptionRenewed/Expired R9) | **§10** out-of-wire |
...
## Open Carried Items (Doc-4I PassA §A8/§A10 — resolved only via named channels, never here)
...
| Check | Result |
|---|---|
| Every Doc-4I contract appears exactly once with a Doc-5I § owner | **PASS** — 32 contracts, partition table + count reconciliation complete |
| Caller-facing + out-of-wire = 32 | **PASS** — 26 + 6 = 32 |
| `resolve_entitlements` / `enforce_quota` classified out-of-wire | **PASS** — definitively out-of-wire (R10); no provisional gate needed |
| `record_payment` fenced as gateway callback (R8) — NOT Doc-2 §8 event | **PASS** — declared R8; out-of-wire §10 |
| Actor-branched contracts: counted once as caller-facing; System leg in-process | **PASS** — R11; 7 actor-branched contracts; each counted once |
| Route namespace = `billing` (no split); matches Doc-5A App B.1 | **PASS** — R3; `billing` route = token; no split |
| Nothing coined (CHK-5A-121/154) | **PASS** — no endpoint/status/header/error-class/slug/POLICY-key/event invented |
| Billing firewall stated (R5) | **PASS** — §3 wire constraint; Appendix A band declared |
| R6 FIXED fence stated | **PASS** — §3 + §8 wire fence |
| Doc-2 §8 events: exactly 3, BC-BILL-2 only | **PASS** — R9; SubscriptionPurchased/Renewed/Expired; others `[ESC-BILL-EVENT]` |
| DF-BILL-1…8 registered; ESC items registered | **PASS** — carried items table complete |
| §3 as mechanism section (no endpoint) | **PASS** — declared |
| No cross-module surface realized | **PASS** — all cross-module interactions by pointer (service/event) |
| Flag-and-halt noted for R1/R8/R10 | **PASS** — all three flag-and-halt declarations present |

---

*End of Doc-5I Structure Proposal v0.1 — structure only. On any conflict, Doc-5A (FROZEN) and the frozen corpus win; flag-and-halt. Next: Independent Hard Review → Structure Patch (if findings) → STRUCTURE FROZEN → content passes.*
