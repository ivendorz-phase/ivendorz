...
| Status | ACTIVE — Content Pass 1 of 3; §0–§3. Independent Hard Review applied (0 BLOCKER/MAJOR; PATCH-01 dual-path rule, PATCH-02 nested-addressing finalized, optional clarifications). m-01 (manage_routing_rule per-variant success) → resolved in Pass-2 §5.7 |
...
- **Async (R1/§10):** **no caller endpoint returns `202`.** Each command commits its own state transition synchronously (`200`/`201`); the matching/routing/comparison **engine runs as downstream System workers (§8)** and its progress is observed via the §4/§7 **reads** (the RFQ state on `get_rfq`; `get_matching_results`; `get_invitation`; `get_comparison_statement`) — the `Doc-5A §10` status-resource pattern, satisfied by reads, never a synchronous facade over the engine. **Comparison generation is engine computation and is never a caller-triggered synchronous operation** (the buyer reads the result via `get_comparison_statement`, §6; the generation is `generate_comparison_statement`, §8).
- **Dual-path leg rule (PATCH-01):** where a contract has **both** caller-facing and internal-service consumption paths, **Doc-5E realizes only the caller-facing wire leg; the internal-service consumption is realized exclusively in §8 and has no HTTP surface** (in-process module composition via `rfq/contracts/` — frozen Doc-5C precedent). This governs `get_rfq`/`list_rfqs`/`get_rfq_version`/`get_matching_results`/`get_routing_log`/`get_invitation`/`list_invitations` (caller leg here; internal leg §8).
- **Binds:** `Doc-5A §5.2/§5.3/§10`, Appendix B.1; Doc-2 §0.3; `Doc-5C` dual-leg precedent.
...
- **Nested / source-addressing [realization convention] — adopted by Doc-5E (PATCH-02, finalized).** The nested-resource and source-command addressing shown in the inventory **is the selected realization** — `get_rfq_version` (`…/rfqs/{id}/versions/{version_id}`), `list_quotations_for_rfq` (`…/rfqs/{id}/quotations`), `get_comparison_statement` (`…/rfqs/{id}/comparison_statement`, **singleton segment — singular by convention**), `list_invitations` (`…/rfqs/{id}/invitations`), `reissue_rfq` (named command on the source RFQ producing a new aggregate → `201`), `manage_routing_rule` (named command, with a create variant → `POST /rfq/routing_rules` `201`). Each is a `[realization convention]` per Doc-5E §0.4 (Doc-5A §5.2/§5.3 are silent on nested child reads, singleton segments, create-via-source-command, and multi-operation single contracts; the conventions contradict nothing upstream). **Alternative flat-collection approaches (e.g. `{rfq_id}`-filtered collections) are retained only as historical review material and are not active candidates unless an architecture review reopens the decision** (`Doc-5_Program_Governance_Note_v1.0 §5`). Pass-2 provides the per-variant §5.7 instantiation (notably `manage_routing_rule`'s create vs update/set_status variants, each with its own path + success code — m-01) with the §0.4 basis stated.
