...
| Field | Value |
|---|---|
| Status | **PROPOSAL — candidate Table of Contents for Doc-4E** (pre-review; not frozen) |
| Module | Module 3 — RFQ Procurement Engine (`rfq` schema) — **the procurement moat** |
| Lifecycle step | Structure Authoring → (next) Independent Hard Review → Structure Patch → Structure FROZEN |
...
**Family-map confirmation (recorded).** This authoring request targets **Doc-4E — RFQ Procurement Engine (Module 3)**, which agrees with the frozen corpus: Doc-4A §1.3 (**Doc-4E = RFQ Procurement Engine, Module 3**), Doc-4A Appendix B (`rfq_` → Doc-4E), Doc-2 §0.3 (`rfq` = Module 3), and Context Pack v3 §3 (Module 3, `rfq` schema, "Authorized next"). No family-map conflict exists; no flag-and-halt is triggered by the request scope. (Doc-4D recorded the inverse reconciliation — a request had mislabeled Module 2 as "RFQ"; that was Board-reconciled to Marketplace, leaving RFQ as Doc-4E. This document occupies that reserved slot correctly.)

**Three governing rules shape this document** (inherited from Doc-4A §0.3 and the Doc-4D structure precedent):
...
3. **Structure only.** This document maps sections; it instantiates no contract, payload, event, slug, audit action, validation, or business rule. Those are the content passes' work, authored against this structure once frozen.

---

# DELIVERABLE A — Document Purpose
...
# DELIVERABLE B — Ownership Statement
...
# DELIVERABLE D — Full Section Tree

> Numbering is hierarchical (`§En` top level; `§En.m`, `§En.m.k` nested). Each top-level section carries **Purpose · Expected content scope · Dependencies · Excluded scope**, matching the frozen Doc-4D structure format. This tree is the candidate Table of Contents; it instantiates nothing.
...
- **DE-4 — Operations boundary (buyer CRM + post-award + vendor CRM).** `buyer_vendor_statuses` (Approved/Conditional/Blacklisted), `private_vendor_records`, `vendor_favorites`, `engagements`, `vendor_leads` are owned by **Operations (Module 4 / Doc-4F)**. The Buyer Filter universe and blacklist floor (Doc-3 §2.1) are **read via the Operations CRM service only**, under the non-disclosure invariant (blacklist exclusion indistinguishable from non-match). Post-award `engagements` and vendor-side `vendor_leads` are created by Operations on RFQ-emitted events (`RFQClosedWon`, `VendorInvited`). **Channel:** read CRM status via service; Operations authors its own consumers; no Operations entity authored.
...
- **Purpose:** Define the bounded contexts **within** Module 3 (Deliverable C), their responsibilities, internal ownership boundaries, and interactions — so content passes place each contract in exactly one context.
- **Expected content scope:** The candidate contexts (BC-1…BC-7 of Deliverable C): **RFQ Authoring & Lifecycle**, **Eligibility & Matching Pipeline**, **Routing/Selection & Distribution**, **Quotation Management**, **Buyer Evaluation & Comparison**, **Procurement Decision & Closure**, **Routing Governance & Control Plane**. For each: responsibilities, internal aggregate ownership, and inter-context interactions (all within RFQ; cross-module interactions deferred to §E9). Confirm the boundary that the **matching/routing engine context lives here**, while the **vendor discovery/profile/attribute context is Marketplace's** (referenced via DE-2).
...
# DELIVERABLE E — Structure Rationale

**Why this structure is implementation-ready.**

1. **It mirrors the proven, frozen Doc-4D structure shape** (§D0–§D13 → §E0–§E14), which has already passed Independent Hard Review and Structure Freeze. Content-pass authors, reviewers, and AI agents already know this skeleton; the per-section quad (Purpose · Expected content scope · Dependencies · Excluded scope) makes each section independently authorable and independently reviewable. The one addition — **§E14 Appendix E (Doc-3 Operational-Rule Binding Index)** — is justified by Module 3's defining trait: unlike Marketplace, the RFQ engine's behavior is specified in a large external frozen document (Doc-3), so an explicit rule-binding index is what keeps Pass-A authors binding by pointer instead of re-deriving.

2. **Ownership is fixed before any contract is written.** Deliverable B + §E2 enumerate every owned aggregate and, just as importantly, every *not-owned* entity with its consuming channel (DE-1…DE-8). This forecloses the single largest risk for the moat module — ownership leakage — at structure time. The moat seam is stated symmetrically with Doc-4D: **DD-2 cedes matching/routing to RFQ; DE-2 confirms RFQ only reads Marketplace's vendor read-model.** No two documents can claim the same contract.

3. **Every behavior has a binding target, and nothing is invented.** The state machines bind to Doc-2 §5.4/§5.5 (including the two patched edges, cited as "as amended by PATCH-D2-01/02"); the pipeline/fairness/capacity/distribution/confidence/quotation/evaluation rules bind to Doc-3 §1–§11; events to Doc-2 §8; audit to Doc-2 §9; slugs to Doc-2 §7; POLICY to Doc-3 §12.2. Where a frozen catalog lacks something (four newer Doc-3-patch behaviors lacking a named §9 audit action; any unregistered POLICY key), the gap is **carried as `[ESC-RFQ-AUDIT]` / `[ESC-RFQ-POLICY]`**, never invented — exactly the discipline that let Doc-4B freeze with carried markers later resolved by additive patch.

4. **The governance firewalls are structural, not aspirational.** The no-pay-to-win invariant, the governance-signal firewall (tier ↛ trust ↛ performance ↛ matching; payment ↛ matching), the non-disclosure invariant (blacklist/deferral indistinguishable from non-match), and "decision-support, never auto-decision" each have a named home (§E5/§E6/§E8/§E13) and an excluded-scope clause that an AI agent can check a contract against.

5. **It is optimized for the stated consumers.** Backend/frontend/QA get a one-contract-per-context placement rule (§E3 + Appendix A skeleton); AI coding agents get §E13's explicit guardrails plus the flag-and-halt obligation; reviewers get the conformance binding maps (Appendix B/D). The structure is implementation-neutral (no transport assumption) and internally complete (no "TBD") per Doc-4A §2 / Authoring Rules 9 & 11.

---

# DELIVERABLE F — Freeze Readiness Assessment

**Is the proposed structure suitable for Structure Review → Structure Freeze → Pass-A Authoring?**

**Assessment: YES — ready to enter Independent Hard Review (the next lifecycle step), and structurally sufficient to reach Structure Freeze and govern Pass-A authoring.**

| Readiness dimension | Status | Basis |
|---|---|---|
| Family-map integrity | **READY** | Doc-4E = RFQ Procurement Engine (Module 3), `rfq_` namespace — confirmed against Doc-4A §1.3/App-B, Doc-2 §0.3; no conflict, no flag-and-halt. |
| Ownership integrity (moat) | **READY** | §E2 + Deliverable B enumerate owned aggregates and all not-owned entities with channels; DE-2 mirrors Doc-4D DD-2; no ownership leakage. |
| Aggregate coverage | **READY** | All five Doc-2 §2 Module-3 aggregates (RFQ, Quotation, Comparison Statement, Routing Rule, Matching Result) mapped to bounded contexts and sections. |
| State-machine coverage | **READY** | RFQ (§5.4 incl. PATCH-D2-01/02), Quotation (§5.5), and invitation lifecycle each have a dedicated home in §E4; terminal states identified; no edge invented. |
| Event coverage | **READY** | Emitted + consumed families (incl. `VendorClaimed` consumed, `RFQMatched`/`RFQRouted` per PATCH-06) structured in §E10; no event coined. |
| Audit coverage | **READY (with carried marker)** | §E12 binds Doc-2 §9 RFQ + Quotation domains; four patch-introduced behaviors carried as `[ESC-RFQ-AUDIT]` (not a blocker — additive-patch channel, the Doc-4B/4C/4D precedent). |
| Authorization coverage | **READY** | §E11 binds the full buyer/vendor slug set + `staff_can_moderate_rfq`; three-layer + delegation via `check_permission`; no slug invented. |
| Cross-module dependencies | **READY** | DE-1…DE-8 cover Platform Core, Identity, Marketplace, Trust, Communication (+ Operations, Admin, Billing) with direction and single-authorship; no leakage. |
| POLICY binding | **READY (with carried marker)** | Doc-3 §12.2 owns the RFQ POLICY inventory; `[ESC-RFQ-POLICY]` carries the Doc-4A §18.2 "must-exist-in-§12.2" obligation for content passes. |
| AI-agent authoring safety | **READY** | §E13 guardrails + per-section Excluded-scope + flag-and-halt make the structure authorable without architectural assumptions. |
| Standards conformance | **READY** | Structure-only (no contract/payload/event/slug/audit/validation/business-rule instantiated); conforms to Doc-4A §0.3, §13, §16, §17, §18, §21 by pointer. |

**Open items at structure freeze (carried, non-blocking):** `[ESC-RFQ-AUDIT]` (four Doc-3-patch behaviors lacking a separately-named Doc-2 §9 audit action) and `[ESC-RFQ-POLICY]` (Doc-4A §18.2 key-existence obligation, satisfied by Doc-3 §12.2 today). Both resolve through additive patches to their owning documents and **do not** reopen Doc-4E — exactly the pattern by which the Doc-3 `core.*` registration satisfied Doc-4B. Consistent with the lifecycle rule that a module may freeze with **no open BLOCKER/MAJOR/MINOR** (NITPICKs/carried markers deferrable), this structure is suitable to proceed: **Independent Hard Review → Structure Patch → Structure FROZEN → Pass-A Authoring.**

> **Recommendation:** advance Doc-4E Structure Proposal v0.1 to **Independent Hard Review**. No conflict with the frozen corpus was found during authoring; no flag-and-halt was triggered.

---

*End of Doc-4E — RFQ Procurement Engine — Canonical Structure Proposal v0.1 (PROPOSAL; pre-review). Structure only — no contract, endpoint, payload, permission, event, validation, or business rule instantiated. Carried freeze-gate dependencies DE-1…DE-8 and escalation markers `[ESC-RFQ-AUDIT]` / `[ESC-RFQ-POLICY]` are documented for carry-through, resolved only via their named upstream channels and never silently. Conforms to Architecture, ADRs, Doc-2 v1.0.3, Doc-3 v1.0.2, Doc-4A/4B/4C/4D v1.0 (all FROZEN). Next lifecycle step: Independent Hard Review.*
