...
> Read this at every session start before any work. Orients a fresh chat to current program state. No prior session memory — everything needed is in the files referenced below. Updated: **2026-06-28** (all programs Doc-2…Doc-8 COMPLETE/FROZEN; **Waves 0–1 DELIVERED** to `main` — Wave 0 Repository Bootstrap (`wave0-complete`) + Wave 1 Foundation / Walking Skeleton (`wave1-complete`) — **current phase = Wave 2 — Core Platform (M0 → M1)**).
