...
| **Authority** | Growth Hub Architecture (**FROZEN** 2026-07-19) §A.4; Board Q-2/Q-14/Q-15/Q-16 + GI-1/2/3; the linked `Doc-2_Patch_v1.0.10` (entities/states/slug/events/audit — the *what*); Doc-4A §21 contract grammar; the frozen `Doc-4C §B.1–B.10` cross-cutting defaults; the public-actor precedent `marketplace.get_public_product_detail.v1`; DC-1 (identity-event-addition channel, `Doc-4C:822`). |
| **Depends on** | `Doc-2_Patch_v1.0.10` (entities/slug/events/audit), `Doc-3_…v1.14_GrowthHub` (the `[DC-5]` POLICY keys; renumbered v1.13→v1.14 per the Doc-4H collision note), `Doc-6C_…v1.0.4` (schema). Atomic fold. |
...
- **Authorization:** Membership active; **Slug `can_manage_growth_invites`** (Doc-2 §7, `Doc-2_Patch_v1.0.10`); Scope active-org (the referrer = the caller's active org); Delegation: not eligible. (§B.10)
...
- **Audit (§B.8):** Domain Organization **"invitation converted (referral attribution)"** (§9;
  `Doc-2_Patch_v1.0.10 §5`); attribution standard; Correlation both.
...
Pins the wire tokens for the Doc-2 §9 Organization additive (`Doc-2_Patch_v1.0.10 §5`):
...
- **Carried realization:** `Doc-6C_…v1.0.4` realizes the tables/RLS/immutability trigger + the two service-role
  ...
  `InvitationConverted`; `Doc-4J`/`Doc-4L` register the events/flow; `Doc-3_…v1.14_GrowthHub` registers the `[DC-5]`
