...
| Status | ACTIVE — Content Pass 2 of 3; §4. **Independent Hard Review applied: m-01 `subject_org_id` reframed as server-validated tenancy (not a query filter); m-02 `REFERENCE`/`422` removed from the safe-read error set (absent ref → empty result); NP-01 VO-2 `Basis` added beside VO-1 `Score`; NP-02 `403`-vs-`404` own-tenancy boundary cited (`Doc-5A §6.3`/`Doc-4A §12.4`).** 0 open BLOCKER/MAJOR/MINOR. Conforms to `Doc-5K_Structure_v1.0_FROZEN.md` (+ Patch CE-01); builds on Pass-1 (§0–§3 + inventory) |
...
- **Cursor-based pagination only — opaque `cursor`, no offset/index** (`CHK-5A-070`; `Doc-5A §8`). The list is **implicitly scoped to the `subject_org_id` tenancy** — this is the **server-validated active-org context, NOT a caller-supplied query param** (§4.1; `Doc-4A §9.7`); it is the artifact-table partition key (`Doc-4K` index `(subject_org_id, entity_ref_id)`), never a wire filter. The actual **query-param filters** are optional `entity_ref_id` / `entity_ref_type` and `include_expired` (default **excludes** expired rows; `include_expired: true` includes them with `is_expired: true`). Counts/items exclude out-of-scope rows identically (R9 — no leakage via totals).
...
- Reads are **safe `GET`s — no idempotency key, no concurrency token, no request body** (`Doc-5A §9` mutation rules N/A). Error classes per **`Doc-5A §6.2`** (by pointer; codes owned by the `Doc-4K` register, `ai_` namespace, `Doc-4A Appendix B.2`): `VALIDATION` → `400` (malformed `{id}` / opaque `cursor` / filter value); `AUTHORIZATION` → `403` **only** where own-tenancy already establishes the right to know the artifact exists, **else `404` collapse** (`Doc-5A §6.3`; `Doc-4A §12.4`; §3.7/R9); `NOT_FOUND` → `404` (cross-tenant). **No `REFERENCE`** — a read does not perform write-side cross-module reference validation; a filter naming an absent `entity_ref` returns an **empty result**, never `422`. No `STATE`/`CONFLICT` (no transition; no write).
