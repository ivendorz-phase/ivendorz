...
> Corpus copy `generatedDocs/Doc-4J_GrowthEvent_Patch_v1.0.1.md`, registered in `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`,
> carried **alongside** its unedited frozen base document(s) — **no frozen file edited in place.**
> Origin/provenance: `governanceReviews/Doc-4J_GrowthEvent_Patch_v1.0.1_PROPOSAL.md` (the authoring + per-patch
...
| **Depends on** | `Doc-2_Patch_v1.0.10` §4 (the canonical §8 producer rows + payloads — the *what*), `Doc-4C_…v1.0.3` (production contracts + §C12.7 outbox flip), `Doc-4H_…` (M6 consumer), `Doc-4I_…` (M7 System branch), `Doc-4L_…v1.0.1` (flow map), `Doc-6C_…v1.0.4` (schema/GI enforcement). *(Doc-3 · Doc-5C · Doc-7E complete the 10-patch atomic set but are not registration dependencies.)* **Atomic fold** — this registration dangles without them. |
...
Doc-2 §8 rows (`Doc-2_Patch_v1.0.10` §4) — never restated; the guard-relevant facts below are summarized
...
(Growth Hub packet §A.3 / `Doc-2_Patch_v1.0.10` §4); this patch performs the catalog's **registration act**
...
|---|---|
| Canonical §8 rows + payloads | `Doc-2_Patch_v1.0.10` §4 |
