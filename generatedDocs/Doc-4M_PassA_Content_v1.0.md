# Doc-4M_PassA_Content_v1.0 — State Machine Contracts

| Field | Value |
|---|---|
| Document | `Doc-4M_PassA_Content_v1.0` — State Machine Contracts — content (Pass-A) |
| Structure authority | `Doc-4M_Structure_FROZEN_v1.0` (FROZEN; sole structure authority — not modified, not reordered, not rescoped) |
| Nature | **Consolidation document.** Creates no state, transition, workflow, business rule, ownership, or event. Consolidates lifecycle authority already defined in the frozen corpus, by reference only. |
...
| Authority | Master Architecture (FROZEN) → ADR Compendium (FROZEN) → Doc-2 v1.0.3 (FROZEN) → Doc-3 v1.0.2 (FROZEN) → Doc-4A v1.0 (FROZEN) → Doc-4B–4L (FROZEN) → `Doc-4M_Structure_FROZEN_v1.0`. On any conflict: **FLAG-AND-HALT**. |
...
| Structure patch corrections inherited | P-4M-01 (M3+M4 §5-vs-§3 classification footers) · P-4M-02 (Trust/Performance Score derived-aggregate annotation) · P-4M-03 (Custom Domain two-row split) · P-4M-04 (Ban Action removed from inventory; footer note) · P-4M-05 (M5 disclaimer strengthened) — all inherited from frozen structure, not re-decided here. |
| Pass-A mission | Populate complete M5 transition inventory for every entity in M3, by reference only. All other sections (M1–M4, M6–M9) carry forward from the frozen structure exactly. No structural change. |
| Status | Pass-A content. |

**Pass-A mission.** Populate the full transition set for every entity enumerated in M3, binding each transition to its From State, To State, Trigger Authority, and Reference Source by pointer only. Carry every escalation marker verbatim. Coin no state; infer no transition; restate no guard, pre/post-condition, or workflow logic. On any apparent structural defect: **FLAG-AND-HALT** (do not edit the frozen structure).
...
**This matrix is the complete Pass-A transition inventory for all M3-enumerated entities.** One transition per row. One trigger authority per row. Guards and pre/post-conditions are in Doc-3 and the owning module's frozen Doc-4 state-transition contracts — not restated here.
...
| RFQ | `shortlisted` | `closed_won` | User (`can_award_rfq`; emits `RFQClosedWon`) | Doc-3 §1.2; Doc-4E (FROZEN) — see M6 |
...
| RFQ Invitation | `selected` | `delivered` | System (delivery dispatch; emits `VendorInvited`) | Doc-2 §3, §8; Doc-4E (FROZEN) — see M6 |
...
| Vendor Lead | `qualified` | `converted` | System (vendor invited to RFQ via `VendorInvited`) | Doc-2 §3 (`vendor_leads`); Doc-4F (FROZEN) — see M6 |
...
| **Verification Record** | | | | |
| Verification Record | `submitted` | `under_review` | System (assigned to verification_tasks queue) | Doc-2 §5.6; Doc-4G (FROZEN) — see M6 |
| Verification Record | `under_review` | `approved` | Admin (`staff_can_verify`; verification decision) | Doc-2 §5.6; Doc-4G (FROZEN) — see M6 |
| Verification Record | `under_review` | `rejected` | Admin (`staff_can_verify`; verification decision) | Doc-2 §5.6; Doc-4G (FROZEN) — see M6 |
...
| Verified Financial Tier | `pending_verification` | `verified` | Admin (`staff_can_verify`; Trust stores; emits `VendorTierChanged`) | Doc-2 §5.6; Doc-4G (FROZEN) — see M6 |
...
| **Subscription** | | | | |
| Subscription | `pending_payment` | `active` | Billing on payment success; emits `SubscriptionPurchased` | Doc-2 §5.7, §8; Doc-4I (FROZEN) — see M6 |
...
| Subscription | `active` | `expired` | System (renewal failed / cancel + period end); emits `SubscriptionExpired` | Doc-2 §5.7, §8; Doc-4I (FROZEN) — see M6 |
...
6. **Structure frozen.** Content is authored against `Doc-4M_Structure_FROZEN_v1.0` without reopening or revising the structure. Any apparent defect found during content authoring is escalated (FLAG-AND-HALT); the structure is not edited without a new Board-authorized patch cycle.

---

## Pass-A Validation

Confirmed for this content pass (consolidation discipline):

| Validation check | Result |
|---|---|
| No new state introduced | PASS — all state strings are navigation keys into Doc-2 §5 / §3; none coined |
| No new transition introduced | PASS — all M5 rows derived by pointer from Doc-2 §5, §3, Doc-3, and owning Doc-4 documents |
| No new workflow introduced | PASS — no workflow narrative, no process flow, no business logic restated |
| No new ownership introduced | PASS — every Owner Module/BC derived by pointer from frozen Doc-4 documents |
| No new event introduced | PASS — all §8 events cited by name from Doc-2 §8; none coined |
| No escalation marker resolved | PASS — M7 carries 5 markers verbatim; none resolved, normalized, or renamed |
| No escalation marker invented | PASS — every M7 marker traces to named frozen source |
| Structure unchanged | PASS — sections M1–M9 match frozen structure layout, order, purpose, and scope exactly |

---

*End of Doc-4M_PassA_Content_v1.0. State Machine Contracts — content (Pass-A) bound to Doc-4M_Structure_FROZEN_v1.0. Sections M1–M9 authored against frozen structure exactly. Consolidation document (non-normative; reference-never-restate). Complete M5 transition inventory for all M3-enumerated entities, by pointer only. Creates no state, transition, workflow, business rule, ownership, or event; coins no lifecycle concept; resolves/normalizes/renames no escalation marker. Lifecycle source of truth: Doc-2 §5 + Doc-3. Authorization grammar: Doc-4A §5/§6. Corpus authority: Master Architecture FINAL → ADR Compendium v1 → Doc-2 v1.0.3 → Doc-3 v1.0.2 → Doc-4A v1.0 → Doc-4B–4L (FROZEN) → Doc-4M_Structure_FROZEN_v1.0. Content document only — no self-review, no findings, no future phases.*
