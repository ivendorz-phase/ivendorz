...
> **✅ STATUS: APPROVED (human — owner/Board ruling 2026-07-03) + FOLDED into the corpus (carried
> alongside the ADR Compendium, rank 1 — same posture as ADR-021/ADR-024, consolidation into
> ADR Compendium v2 deferred to a future human re-freeze).**
> This is the corpus copy `generatedDocs/ADR-025_Marketplace_Public_URL_Law.md`, registered in
> `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`.
> Origin/provenance: `governanceReviews/ADR-025_Marketplace_Public_URL_Law_PROPOSAL.md`.
> Realizes instrument **E-3** of the `ESC-7-API-PRODDETAIL` Board Resolution
> (`R-ESC7-PRODDETAIL-FREEZE`, Annex E Decisions #4–#5). Folded together with the linked E-1 pair
> (`Doc-4D_PublicProductDetail_Patch_v1.0.3` + `Doc-5D_PublicProductDetail_Patch_v1.0.1`) and E-2
> (`Doc-3_Policy_Key_Registration_Patch_v1.11_PublicReadRateLimit`) — the full Annex E instrument
> set landed together at this fold.

| Field | Value |
|---|---|
| Number | **ADR-025** (next free; ADR Compendium scope stops at ADR-020; ADR-021/ADR-024 carried alongside; ADR-022/023 raised, folds pending; ADR-019 reserved/do-not-backfill) |
| Date | 2026-07-03 |
...
| Realized by | `Doc-4D_PublicProductDetail_Patch_v1.0.3.md` (`canonical_url` field, Class K) + `Doc-5D_PublicProductDetail_Patch_v1.0.1.md` (wire shape) — both already reference this ADR by pointer (`E-3/ADR-025`), folded together. Doc-7 leg (FE canonical-URL builder realization) is a separate future milestone, explicitly deferred here — same deferral pattern ADR-024 used for FE-PUB-10. |
...
| Authority | CLAUDE.md §7 (authority order), §8 (architecture-affecting artifacts → human approval — satisfied 2026-07-03), §11 (Flag-and-Halt / reference-never-restate), §13 (Raise ≠ Accept; Validate-Findings gate) |

---

## Context

`marketplace.get_public_product_detail.v1` (instrument E-1, `Doc-4D_PublicProductDetail_Patch_v1.0.3.md`
and `Doc-5D_PublicProductDetail_Patch_v1.0.1.md`) declares a `canonical_url`
field — opaque, computed-at-read, never stored — and both patches point at
this ADR by name: *"emitted only by the canonical URL builder (E-3/ADR-025 — referenced by
pointer, never restated here)"*. This ADR is what that pointer resolves to.
...
field). **FE-PUB-05 un-gates at this fold** — Annex E items E-1/E-2/E-3 have all landed together.
...
*ADR-025 (APPROVED — FOLDED 2026-07-03) — composes with ADR-024 (Vendor leg, unchanged) and
ADR-022 (route IA), supersedes nothing. Realizes E-3 of the `ESC-7-API-PRODDETAIL` resolution;
unblocks E-1's `canonical_url` contract freeze. Doc-7 leg deferred to a future milestone.*
