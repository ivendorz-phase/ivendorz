...
| Status | ACTIVE — Content Pass 1 of 3; §0–§3. Independent Hard Review applied (MAJOR-01 §3.4 Internal-Service dual-path fence; MAJOR-02 §1.4 DD-7 freeze-scope; MIN-01 §2.1 DD-2 no-matching-surface; MIN-02 §2.6 projections never merged; MIN-03 §3.6 Billing single-authority; MIN-04 §3.5 delegation via check_permission only; NP-01 §2 ordering; NP-02 §3.4 projection origin; NP-03 §2.1 search-as-read). Round-2: M-01 §3.1 DD-1 mislabel removed (Identity = `Doc-4C §C3` consumed, no DD number); O-01 §1.3 cross-module §9 reference removed; NP-01 §2.5 `vendor_directory` §0.4 annotation. 0 open BLOCKER/MAJOR/MINOR. Conforms to `Doc-5D_Structure_v1.0_FROZEN.md` |
...
- **Carried (Doc-4D Appendix C — by pointer; resolved only via named channels):** **DD-1** Trust verification · **DD-2** RFQ matching logic (M2 owns read-model only) · **DD-3** Admin ban decision · **DD-4** Admin category governance · **DD-5** Billing entitlement · **DD-6** `marketplace.*` POLICY keys (**content-freeze gate** — additive Doc-3 §12.2 patch) · **DD-7** `vendor_claim_records` tenancy (**cross-frozen-doc, Board-gated**) · **DD-8** ban-lift non-implementable (no `VendorBanLifted` event) · `[ESC-MKT-AUDIT]` (nearest Doc-2 §9 action).
- **DD-7 freeze-scope (explicit — MAJOR-02):** **DD-7 blocks finalization of the `claim_vendor_profile` realization only. It does NOT block Doc-5D authoring, Hard Review, the Freeze Audit, or freeze of any unrelated contract.** The Doc-2 §6 vs §10.3/§3.3 tenancy ambiguity is a cross-frozen-doc conflict resolved only via an additive Doc-2 reconciliation (architecture-touching → human/Board); no ownership decision is made here. Every other M2 contract proceeds independently.
...
> **Inventory ordering is non-authoritative** (informational aggregation). Section ownership (the structure § column) is authoritative; on conflict the partition table wins.
...
- **DD-2 moat boundary (MIN-01):** **no caller-visible matching, ranking, recommendation, or routing engine surface exists.** That logic is RFQ-owned (`Doc-4E`); M2 owns only the `vendor_matching_attributes` read-model (internal-service, out-of-wire §9). Doc-5D authors no matching/eligibility/ranking/selection endpoint.
- **Search-as-read (NP-03):** `search_catalog` (§8) is a `GET` read — **a search remains a read operation and never creates server state**; query/filter parameters are request inputs, not mutations.
...
| 63 | `marketplace.list_vendor_directory.v1` | Public | `GET /marketplace/vendor_directory` *(anonymous; directory-as-read — §0.4)* | N (public) | `200` | §8 |
...
- **Public/anonymous** caller carries **no `Authorization`** — discovery/directory/public-profile and published catalog reads are open (`Doc-5A §7.1`; R2). **User** carries the **`Authorization`** bearer — **authentication only** (credential/session mechanics out of scope — Identity/Supabase Auth, consumed by pointer per `Doc-4C §C3` / Doc-5C; not a Doc-5D carried DD item). **Admin** carries the bearer; actor type is server-determined (§3.2).
...
- **Internal-Service dual-path fence (MAJOR-01):** Where a Doc-4D contract has **both** a caller-facing path and an Internal-Service consumption path, **Doc-5D realizes only the caller-facing wire leg; the Internal-Service consumption is realized exclusively in §9 and creates no additional HTTP surface** (in-process module composition via `marketplace/contracts/`, never HTTP — frozen Doc-5C/5E precedent). The `projection-gated` / `RFQ-gated leg` notes in §2 (e.g. `get_spec_document`, the matching read) mark the caller leg only; any Internal-Service consumption is §9.
- **Projection classification origin (NP-02):** the projection class of each read **originates from Doc-4D's visibility rules and does not create new contracts** — it is a wire-realization of an existing Doc-4D contract, not a new surface.
- **Binds:** `Doc-5A §6.3`; `Doc-4A §7.5`; `Doc-2 §0.2` (exclusion of soft-deleted/unpublished/retired); Doc-4D §D6/§D7.3; structure §1 (dual-path rule).

### 3.5 Authorization Realization (§7.5) — single root
- Authorization is **server-side** — three-layer check (active Membership + Permission Slug + Resource Scope) **OR** an active delegation grant — resolved from §3.1–§3.3 context via Identity **`check_permission`** (consumed; `Doc-5A §7.5`; `Doc-4A §6.1`). **`check_permission` is the sole authorization authority consumed by M2; no parallel or shadow authorization path is permitted** (`Doc-4A §5.3/§6`). **Delegation grants are resolved exclusively through Identity `check_permission` — Doc-5D performs no direct grant inspection and accepts no grant id / scope / attribution as a wire input** (`Doc-4A §6B/§9.7`). Slugs (`staff_can_manage_categories`, vendor/profile slugs, …) are **never** wire inputs (`Doc-4A §6.2`); favorites are **membership-only (no slug)** (DD-6 note). `[ESC-MKT-AUDIT]` / `staff_*` gaps (DD-3/DD-4) bound by pointer (R4).
...
- **Non-disclosure (R9):** no blacklist / private-CRM / Buyer-Vendor-Status / banned / suspended / soft-deleted fact is ever surfaced on a Marketplace-facing read; a protected-fact-gated read collapses to a uniform **`NOT_FOUND`** identical in status, body, and timing to genuinely-absent (no side-channel). Banned/suspended profiles drop from §8 discovery reads. **Entitlement gating (R8):** the advertisement and custom-domain surfaces **consume** Billing (`Doc-4I`) entitlement checks at the gate; **denial collapses to `NOT_FOUND`** (not a distinguishable `403` that would leak the gated resource's existence). **Billing entitlement checks are consumed only through Billing-owned authorities and are never duplicated, re-modeled, or shadow-evaluated in M2** (DD-5; `Doc-4A §4.1/§4.3`). Doc-5D authors no Billing contract; ad purchase is Billing-owned, referenced by bare UUID (`Doc-4D §D7.4`; DD-5).
