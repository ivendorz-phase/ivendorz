# Doc-4K_FROZEN_v1.0 — Module 9 — AI Layer — API & Integration Contracts
...
Baseline: Doc-4K_PassB_Content_v1.0 with Doc-4K_PassB_Patch_v1.0 corrections
          applied inline. This is the authoritative consolidated text.

Patch corrections applied:
  F4K-PB-01 (MINOR) — B-AI-1-1 §(3): JSON-path projection rows removed;
             result_jsonb row points to §K5-VO; trailing note extended.
  F4K-PB-02 (MINOR) — B-AI-1-2, B-AI-2-2, B-AI-3-2, B-AI-4-2 §(11)/(12):
             Response schema disambiguated (null = not-found only);
             E-AI-x-014 NOT_FOUND success rows added.
  F4K-PB-03 (NITPICK) — B-AI-2-4, B-AI-3-4, B-AI-4-4 §(3–5):
             Target table + sweep predicate named explicitly.
  F4K-PB-04 (NITPICK) — B-AI-1-2, B-AI-2-2, B-AI-3-2, B-AI-4-2 §(9):
             PK index noted as implicit / name implementation-defined.
  F4K-PB-05 (NITPICK) — deferred to Consolidation; recorded, not applied.
...
Deferred finding (traceable):
  F4K-PB-05 (NITPICK) — per-contract ESC-marker citation depth → Consolidation

Integrity (all 11 Final Freeze Audit domains: PASS):
...
| Field | Value |
|---|---|
| Document | Doc-4K — **FROZEN v1.0** — consolidated hardened contract specifications for Module 9 — AI Layer (`ai` schema, `ai_` namespace), bounded contexts **BC-AI-1 / BC-AI-2 / BC-AI-3 / BC-AI-4** |
| Lifecycle stage | **FROZEN** — Pass-B corrections applied inline; this is the authoritative consolidated text for Module 9 |
...
| Audience | Claude Code / Cursor / OpenAI Codex / backend / frontend / QA / Solution Architects — implementation-ready |

**Module 9 mission.** Provide advisory-only AI-derived artifacts (recommendations, predictions, classifications, similar-vendor results) for the iVendorZ platform. AI suggests; humans and authoritative module contracts decide. Module 9 writes only its own `ai.*` derived tables. No slug, event, audit action, or POLICY key is coined. No redesign of Module 9. On any corpus conflict: **FLAG-AND-HALT**.

---

## §B — Cross-Cutting Conventions

### B.1–B.9 — Pass-A Conventions (inherited, not restated)

All Pass-A §B conventions apply without modification.

### B.10 — Idempotency

All `generate_*` (21.4 Command / 21.5 System) contracts are idempotent on `(subject_org_id, entity_ref_id, model_version)`: a re-run with the same key upserts the existing row rather than inserting a duplicate. Idempotency key = `ai_idempotency_key` = UUIDv7 per-request (Doc-4B UUIDv7 service); stored on the artifact; duplicate detection window = TTL of the artifact.

### B.11 — Concurrency

All `generate_*` contracts use **last-writer-wins optimistic upsert** on the artifact row (no version-lock required — artifacts are regenerable, non-authoritative). Concurrent regenerations of the same artifact key are safe: the later write replaces the earlier; no business-invariant is violated.

### B.12 — Retention & TTL

Cache TTL / `expires_at` window values are runtime-tunable POLICY values (Doc-3 §12.2). **No POLICY key is coined here** (`[ESC-AI-POLICY]`); the placeholder `policy.ai.<bc>.ttl_seconds` per BC is an additive-channel carry. Hard delete on expiry (cache semantics, Doc-2 §10.10). Retention = TTL only. Audit records of generation actions are retained per `core.audit_records` policy (Platform Core/Doc-4B — not governed here).

### B.13 — Indexes

Minimum required index per artifact table: `(subject_org_id, expires_at)` for expiry-sweep queries; `(subject_org_id, entity_ref_id)` for point-lookup and filtered listing; `(expires_at)` partial index for the cache-maintenance sweep.

### B.14 — Error Classification

Three-tier error model: **VALIDATION** (client input malformed; 4xx-class), **PERMISSION** (entitlement check failed; 403-class), **SYSTEM** (infrastructure/derivation failure; 5xx-class). AI-derivation model failure is SYSTEM. Corpus-absent error action codes carry `[ESC-AI-AUDIT]`. NOT_FOUND is a success-path documentation tier (200-class outcome in `get_*` contracts), not a fourth error class.

### B.15 — Value Objects (BC-AI-1 only)

Doc-2 §2 enumerates two value objects on the Recommendation aggregate: **Score** (numeric confidence value; range 0.0–1.0; immutable per generation; stored in `result_jsonb`) and **Basis** (explanation string; human-readable derivation rationale; immutable per generation; stored in `result_jsonb`). Specified in §K5-VO below. No value object is enumerated for BC-AI-2, BC-AI-3, or BC-AI-4 in Doc-2 §2; none is invented here.

### B.2 — Invariant 12 (Master Architecture §18)

AI-Agent contracts are read-only or advisory with respect to authoritative business data; the AI Layer's only writes are to its own `ai.*` derived tables. Any authoritative write that results from an AI advisory is executed by a User or System actor contract in the owning module and attributed to that actor — never to the AI Agent, and never authored here.

---

## §K5-VO — Value Object Specifications (BC-AI-1 only)

*Doc-2 §2 enumerates Score and Basis on the Recommendation aggregate.*
...
## §K6 — Hardened Contract Specifications

Per-contract 12-section hardening. Sections: (1) Identity (2) Purpose (3) Field Registry (4) Value Objects (5) Read Model (6) Idempotency (7) Concurrency (8) Retention & TTL (9) Index Strategy (10) Request Schema (11) Response Schema (12) Error Matrix.
...
Not applicable — this contract writes. Read models in B-AI-1-2 (get) and B-AI-1-3 (list).
...
- **Behavior:** upsert semantics — if a row with matching key exists, refresh `result_jsonb`, `generated_at`, `expires_at`, `ai_idempotency_key`; do not insert duplicate. Different `model_version` creates a new row (model upgrade path).
- **Duplicate window:** TTL of the artifact; expired rows not considered for deduplication — regeneration after expiry is a fresh insert/upsert.

**(7) Concurrency**

Last-writer-wins optimistic upsert (B.11). No version-lock. Concurrent regenerations of the same `(subject_org_id, entity_ref_id, model_version)` key are safe — the later write prevails; no business-invariant violated. Database upsert is atomic (`INSERT ... ON CONFLICT DO UPDATE`).
...
**Value objects:** Score (VO-1) + Basis (VO-2) for BC-AI-1 only (Doc-2 §2); none invented for BC-AI-2/3/4.
...
*End of Doc-4K_FROZEN_v1.0. Module 9 — AI Layer. FROZEN 2026-06-20. 16 hardened contracts (4 generate + 4 get + 4 list + 4 expire) across BC-AI-1 Recommendation · BC-AI-2 Prediction · BC-AI-3 Classification Result · BC-AI-4 Similar Vendor Result. Patch corrections F4K-PB-01 through F4K-PB-04 applied inline. F4K-PB-05 deferred to Consolidation (traceable). Frozen Board decisions Q1/Q2/Q3 applied without modification. Moat: AI suggests; RFQ decides. Firewall: AI confidence ≠ Trust/Verification/Performance/Governance score. Corpus authority: Master Architecture v1.0 FINAL → ADR Compendium v1 → Doc-2 v1.0.3 → Doc-3 v1.0.2 → Doc-4A → Doc-4B–4J (FROZEN) → Doc-4K_Structure_FROZEN_v1.0 → Doc-4K_PassA_Content_v1.0 → Pass-A Approved Corpus → Doc-4K_PassB_Content_v1.0 → Doc-4K_PassB_Patch_v1.0 → Doc-4K_PassB_Patch_Verification_v1.0 → Doc-4K_Final_Freeze_Audit_v1.0. Freeze verdict: FREEZE APPROVED.*
