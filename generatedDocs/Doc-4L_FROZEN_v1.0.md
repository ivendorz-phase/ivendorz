# Doc-4L_FROZEN_v1.0 — Permission Authority Matrix

| Field | Value |
|---|---|
| Document | `Doc-4L_FROZEN_v1.0` — Permission Authority Matrix |
| Status | **FROZEN** |
| Freeze Date | 2026-06-20 |
| Freeze Authority | Doc-4L_Final_Freeze_Audit_v1.0 — FREEZE APPROVED WITH NITPICKS (BLOCKER=0 · MAJOR=0 · MINOR=0 · NITPICK=1) |
| Board Assessment | APPROVE — BLOCKER=0 · MAJOR=0 · MINOR=0 · NITPICK=1 (F4L-PA-01 = Board Observation; no action required) |
| Nature | **Consolidation document.** Non-normative. Creates no permission, ownership, role, actor, business rule, or state transition. Consolidates permission authority already defined in the frozen corpus, by reference only. |
...
| Authority | Master Architecture (FROZEN) → ADR Compendium (FROZEN) → Doc-2 v1.0.3 (FROZEN) → Doc-3 v1.0.2 (FROZEN) → Doc-4A v1.0 (FROZEN) → Doc-4B–4K (FROZEN) → `Doc-4L_Structure_FROZEN_v1.0` → `Doc-4L_PassA_Content_v1.0`. On any conflict: **FLAG-AND-HALT**. |
| Permission source of truth | **Doc-2 §7 — Permission Mapping** (the canonical slug catalog). Owner-BC attribution by pointer to each module's frozen Doc-4 document. |
| Structure corrections applied | P-4L-01 (`can_submit_verification` L3 ref → ownership only; dependency in L5-1) · P-4L-02 (`can_create_documents` → single BC-OPS-2 per Doc-4F) · P-4L-03 (L6 preamble marker-format clarification) |
| Deferred observation | F4L-PA-01 (NITPICK) — L5-1 entity-owner Reference Source (`Doc-4D DD-1` → should cite `Doc-4G`); Board Observation; correction deferred to next structure patch cycle; not freeze-blocking |
| Lineage | `Doc-4L_Structure_Proposal_v0.1` → `Doc-4L_Structure_Patch_v1.0` → `Doc-4L_Structure_FROZEN_v1.0` → `Doc-4L_PassA_Content_v1.0` → Pass-A Hard Review (APPROVE) → Board Assessment (APPROVE) → `Doc-4L_Final_Freeze_Audit_v1.0` (FREEZE APPROVED) → **`Doc-4L_FROZEN_v1.0`** |

**Freeze certificate.** This document is the authoritative frozen consolidation index for the iVendorZ Permission Authority Matrix. It supersedes all prior Doc-4L drafts, proposals, patches, and pass documents. The content is locked. No permission, ownership, role, actor, business rule, or state transition is introduced. The slug set is exactly the Doc-2 §7 catalog. On any apparent conflict with a frozen corpus source, the frozen source governs and the conflict is escalated — this index is corrected, never the frozen corpus.
...
> **Deferred observation (F4L-PA-01 — NITPICK):** L5-1 Reference Source cites `Doc-4D (FROZEN) DD-1` for the entity-owner side of this seam. The entity owner of `verification_records` is Trust (Module 5); the more precise entity-owner reference is `Doc-4G (FROZEN)`. The Entity Owner column (Trust) and the primary ownership lookup (L3 `can_submit_verification` → Doc-4C) are both correct; only the L5-1 Reference Source pointer is imprecise. Board Observation accepted; correction deferred to next structure patch cycle. — *F4L-PA-01 / Board Assessment 2026-06-20*
...
6. **FROZEN.** This document is Doc-4L_FROZEN_v1.0. It is the authoritative, locked consolidation index for the iVendorZ Permission Authority Matrix. It supersedes all prior Doc-4L drafts. No content may be modified without a new Board-authorized structure patch cycle.

---

*End of Doc-4L_FROZEN_v1.0. Permission Authority Matrix — FROZEN 2026-06-20. Consolidation document (non-normative; reference-never-restate). 35 tenant + 7 canonical staff slugs (Doc-2 §7) consolidated by reference. Owner-BC attribution by pointer to Doc-4B–4K (FROZEN). 4 pending least-privilege staff slug names carried in L6 only (not catalogued). Creates no permission, ownership, role, actor, business rule, or state transition; coins no slug; resolves/normalizes/renames no escalation marker. One deferred Board Observation (F4L-PA-01 NITPICK, L5-1 entity-owner reference pointer imprecision; next structure patch cycle). Permission source of truth: Doc-2 §7. Authorization grammar: Doc-4A §5/§6. Corpus authority: Master Architecture FINAL → ADR Compendium v1 → Doc-2 v1.0.3 → Doc-3 v1.0.2 → Doc-4A v1.0 → Doc-4B–4K (FROZEN) → Doc-4L_Structure_FROZEN_v1.0 → Doc-4L_PassA_Content_v1.0. Doc-4L is hereby designated: Doc-4L_FROZEN_v1.0.*
