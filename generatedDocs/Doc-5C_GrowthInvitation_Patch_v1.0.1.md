...
> Corpus copy `generatedDocs/Doc-5C_GrowthInvitation_Patch_v1.0.1.md`, registered in `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`,
> carried **alongside** its unedited frozen base document(s) — **no frozen file edited in place.**
> Origin/provenance: `governanceReviews/Doc-5C_GrowthInvitation_Patch_v1.0.1_PROPOSAL.md` (the authoring + per-patch
...
| **Depends on** | `Doc-4C_…v1.0.3` (contracts) · `Doc-3_…v1.14_GrowthHub` (the resolve rate-limit key + the **`identity.growth_invite_dedup_window`** idempotency key + quota/TTL keys — the DC-5-class finalization gate) · `Doc-2_Patch_v1.0.10` · `Doc-6C_…v1.0.4` (persistence). **Atomic fold.** |
...
- **Path grammar (Doc-5A §5.3; the frozen §2.1 note):** `growth_invitations` = the owning entity table
  (Doc-2 v1.0.10 §1), rendered plural — grammar-conformant. Row 37's `resolve` is a collection-level
...
- **Row 36 — idempotency · audit · events (Review-A MAJOR-1 / Review-B F-1/F-2):**
  - **`Idempotency: required`** (Doc-4C §C13) → the **`Idempotency-Key` header is mandatory** (Doc-5A §9;
    ...
    (`Doc-3_…v1.14_GrowthHub` §3A key 1), by pointer — never a literal.
  ...
  - **Events:** emits `InvitationIssued` **iff targeted** (Doc-2 v1.0.10 §4 / Doc-4C §C13) via the M0
...
- Row 37 binds the registered **`identity.growth_invite_resolve_rate_limit`**
  (`Doc-3_…v1.14_GrowthHub` §3A key 5 — the v1.11 `PublicReadRateLimit` model: the POLICY exists; no
...
- **R6 effective-reading note (rationale refresh, conclusion unchanged):** R6's rationale ("no `identity`
  domain event" — DC-1) is superseded by `Doc-2_Patch_v1.0.10` §4 (M1's two §8 events). The **conclusion
...
- **DC-5 note:** the frozen DC-5 gate ("contracts not finalized until Doc-3 §12.2 registration") is
  satisfied for the growth contracts by `Doc-3_…v1.14_GrowthHub` at the atomic fold.
