...
| `Doc-8A_Structure_v1.0_FROZEN.md` (Test & Conformance Realization Metastandard) | STRUCTURE FROZEN | v1.0 | ✅ | The test-program metastandard (the Doc-5A/6A/7A analog); realizes the frozen *what*-corpus (Doc-2/3/4) + the Doc-5/6/7 realization contracts as executable conformance suites; effective = `Doc-8A_Structure_Proposal_v0.1` + `Doc-8A_Structure_Patch_v0.1.1`; fixes R1–R12; **test-discipline partition** (Doc-8B Foundation/Harness *frozen first* + 8C Contract/API + 8D Persistence/Migration/RLS + 8E Domain/Invariant/State-Machine + 8F Integration/Event-Flow + 8G Frontend/E2E); cross-cutting conformance-concern allocation table; Appendix A = 9 `CHK-8-xxx` bands (per-suite freeze gate); the RLS byte-equivalence gate `Doc-6A R8/§4` + migration gate `Doc-6A §11` inherited. **Conformance harness yet subordinate to its oracle**; coins nothing. **Content passes NOT STARTED.** Governs Doc-8B…8G |
| `Doc-8A_Structure_Proposal_v0.1.md` · `Doc-8A_Structure_Independent_Hard_Review_v0.1.md` · `Doc-8A_Structure_Patch_v0.1.1.md` · `Doc-8A_Structure_Patch_Short_ReReview_v0.1.1.md` · `Doc-8A_Structure_Freeze_Audit_v1.0.md` | PROVENANCE | v1.0 | — | Doc-8A structure authoring history (Pass → Board Hard Review [2 MAJOR + 2 MINOR + 2 NIT + 1 OBS resolved, 1 REJECTED] → Structure Patch + short re-review PASS → Structure Freeze Audit PASS) |
