...
| `Doc-5F_Structure_v1.0_FROZEN.md` (M4 `operations` API realization) | STRUCTURE FROZEN | v1.0 | ✅ | Canonical TOC realizing Doc-4F (50 contracts: 46 caller-facing + 4 out-of-wire) on HTTP; the post-award ERP-Lite layer; two-sided tenant User (no Admin/public); R1–R10 (non-disclosure load-bearing R5, money-boundary R8, async doc-gen R9); DF-1…DF-8; partition independently verified. Content passes next (3). `[ESC-OPS-POLICY]` (create `operations.*` namespace) + `reference_id` = content-freeze obligations |
| `Doc-5F_Structure_Proposal_v0.1.md` | PROVENANCE | v0.2+r2 | — | Doc-5F structure authoring history (Board Hard Review + round-2 ADD-1/ADD-2) |
