...
| **Depends on** | `Doc-2_Patch_v1.0.10` §4 (the `InvitationConverted` event), `Doc-4C_…v1.0.3` (§PROV-EXT producer), `Doc-4J_…v1.0.1` (catalog registration), `Doc-4L_…v1.0.1` (flow row L9-2). **Atomic fold** — the branch's trigger does not exist without them. |
...
> **Events — Consumes `InvitationConverted`** (M1-owned; Doc-2 §8 as amended by `Doc-2_Patch_v1.0.10` §4;
...
only by the new consumer. Carried realization: producer/event = `Doc-2_Patch_v1.0.10` §4 + `Doc-4C_…v1.0.3`;
