...
/**
 * `track_referral` SYSTEM EVENT-CREATE branch input (Doc-4I_GrowthReferral_Patch_v1.0.1 §1 — the added
 * System branch of the SAME contract-ID `billing.track_referral.v1`; F4I-PA-M1). Q-15 guard 3: BOTH org
 * ids are taken from the consumed `InvitationConverted` payload — no caller override; `source_event_id
 * = event_id` of the consumed event (the §HB-6.1-precedent idempotency key) — REQUIRED on this branch,
 * absent on the org branch (the §5 Inputs-row overlay).
 */
export interface TrackReferralFromEventInput {
  /** From the event payload `referrer_organization_id` (Doc-2_Patch_v1.0.10 §4). */
  referrerOrganizationId: string;
  /** From the event payload `referred_organization_id` (Doc-2_Patch_v1.0.10 §4). */
  referredOrganizationId: string;
  /** `= event_id` of the consumed `InvitationConverted` (required on this branch only). */
