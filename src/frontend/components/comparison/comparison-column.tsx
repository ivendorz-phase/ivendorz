// Doc-7B kit — comparison COLUMN builder. Promoted from the buyer-scoped `P-BUY-15` realization (Shared
// Platform Component Registry §4.2 CTO override — 2026-07-03). Builds the `DataListTable` columns for the
// transposed comparison matrix: a sticky first column for the attribute LABEL (rendered as a row header by
// the table), then one column per SUPPLIER in the SYSTEM-supplied order. The order is the System's — this
// builder maps it 1:1 and NEVER re-orders/re-ranks the suppliers (R6 / GI-04). PRESENTATION-ONLY.

import type { DataColumn } from "@/frontend/components/data-list-table";
