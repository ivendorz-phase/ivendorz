...
| **`ESC-IDN-DISPLAYNAME`** | **Doc-4C ↔ Doc-2/Doc-6C divergence (RV-0148 MINOR-4, 2026-07-09):** Doc-4C §C3's `get_user` projection (and §C4's `update_user_profile` wire) reference a `display_name` field that **no realized column carries** — Doc-2 §10.2 (line ~717) and Doc-6C `identity.users` both lack it; the W2-IDN-3 `get_user` honestly omits it | `get_user` projects the realized columns only; the omission is commented with this handle; **W2-IDN-6.1 (`update_user_profile` wire) is GATED on this handle for that field** — it must not coin the column nor silently drop the frozen wire field | Human Board — rule: (A) additive Doc-2/Doc-6C patch realizing `display_name`, or (B) additive Doc-4C editorial patch removing the field from §C3/§C4 |
