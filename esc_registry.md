...
| **`ESC-IDN-SLUGCOUNT`** | **Frozen-vs-frozen permission-slug count mismatch (W2-IDN-2 Flag-and-Halt, 2026-07-09):** Doc-2 §7 (base text through the v1.0.5 patch chain) **enumerates 43 distinct slugs (36 tenant + 7 staff)**; Doc-6C §5.1 + Doc-6C Pass-3 CHK-6-062 + `backend_execution_playbook.md` §5 all **assert "45 (38 tenant + 7 staff)"** — none enumerates the 38; all cite Doc-2 §7 as the exhaustive list, which does not substantiate 45. Patch chain v1.0.3/4/5, PATCH-10, and Master Architecture §13 ruled out as sources of the missing 2. Evidence: `governanceReviews/milestones/w2-idn-2/HANDOFF-NOTE.md` §3/§4 + Board packet `governanceReviews/BOARD-PACKET-IDN-SLUGCOUNT_v1.0.md` | **W2-IDN-2 SUSPENDED at `c21c38f`** — zero seed rows written; nothing coined; even the enumerated 43 are NOT seeded until the ruling | **Human Board** — neutral options: **(A)** Doc-2 §7's enumerated 43 = truth → additive editorial patch corrects the "45/38" assertions in Doc-6C §5.1/CHK-6-062 + playbook; **(B)** 45 intended → additive **Doc-2 §7 patch enumerating the missing 2 tenant slugs** (human-approved architecture patch — never a seed-time invention) |
