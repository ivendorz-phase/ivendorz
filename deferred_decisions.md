...
| **DEF-03** | **Light-default code reconciliation** (`app/globals.css` / `tailwind.config.ts` shipped dark-default; `DP §3.1` sets light-primary) | **RESOLVED** — opened & executed by the Wave-2 FE-foundations WP (2026-06-30). A semantic-token layer (`:root` light + `.dark`) now drives the kit; `body` defaults light; dark is a first-class override (no switcher UI yet). | — (residual: the legacy dark-oriented `.iv-*` utility classes retire on kit adoption) | `app/globals.css` (semantic layer + `.dark`), `tailwind.config.ts` (shadcn color keys); kit at `src/frontend/` |
