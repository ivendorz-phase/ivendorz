...
| **D7** ✅ | **Resume M1** | ✅ **DONE (2026-06-30).** The M1 buyer-profile write vertical (`upsert_buyer_profile`) shipped **with** audit — Command → `appendAuditRecord()` → Repository → Transaction; canonical actions `buyer_profile_created`/`buyer_profile_updated` (Doc-2 v1.0.4 + Doc-4C v1.0.2). Full backend (command/repo/facade/route/server-handler) + account form + **6/6 slice tests + full suite 58/58 GREEN vs real PostgreSQL**; both transaction↔audit invariants proven. Reports: `D7_Traceability_Report_v1.0.md` · `D7_Conformance_Report_v1.0.md`. (Audit-action governance prerequisites folded first: Doc-2_Patch_v1.0.4 + Doc-4C_BuyerProfileAuditToken_Patch_v1.0.2.) |
