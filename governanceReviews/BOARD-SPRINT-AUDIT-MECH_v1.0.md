...
| **Status** | **OPEN — ACTIVE. D1 ruled R-b; D2/D3 + ADR-021 APPROVED AS DRAFT by the Board (2026-06-30)** — see [`ESC-W2-AUDIT-RLS` §7](ESC-W2-AUDIT-RLS_v1.0.md). The drafts (`D2_Doc-4B_AuditAppendRLS_Additive_Patch_PROPOSAL.md`, `D3_Doc-6B_AuditInsertPolicy_Additive_Patch_PROPOSAL.md`, `ADR-021_Audit-Records-RLS-Asymmetry_PROPOSAL.md`) meet the Board's three pre-freeze conditions (ADR extraction · D3 normative/appendix split · normative Deployment Constraint) and are **READY FOR HUMAN REVIEW/FREEZE**. The formal fold into the frozen corpus (`generatedDocs/` + `00_AUTHORITY_MAP.md` + version bump) is a **human action** (§7/§8) — not done by AI. D4 direction confirmed (keep SELECT, eliminate RETURNING, app-side UUID, atomic, no `SECURITY DEFINER`/elevation). D4–D7 pending. No frozen doc edited; no D4 code written. |
...
| **D2** ▶️ | **Additive Doc-4B patch** | ▶️ **DRAFTED as proposal — awaits HUMAN approval.** `D2_Doc-4B_AuditAppendRLS_Additive_Patch_PROPOSAL.md` records the §B10/§17.1 audit-append RLS posture (admission via the D3 INSERT policy; read stays staff-only; atomicity/no-elevation preserved). No contract-behavior change; no frozen edit. On approval folds as `Doc-4B_AuditAppendRLS_Patch_v1.0.1.md`. **Done when** human-approved + frozen via the corpus process. |
| **D3** ▶️ | **Additive Doc-6B patch** | ▶️ **DRAFTED as proposal — awaits HUMAN approval.** `D3_Doc-6B_AuditInsertPolicy_Additive_Patch_PROPOSAL.md` adds the append-only context-bound `FOR INSERT` policy (parent + default + every monthly partition; `actor_type='user'` lowercase) to `core.audit_records` (Doc-6B §2.2/§4.1), staff read backstop + CR4′ immutability untouched. On approval folds as `Doc-6B_Structure_Additive_Patch_v1.0.1.md`. **Done when** human-approved + frozen via the corpus process. |
| **D4** | **M0 implementation** | The mechanism is realized **once** in `src/modules/core`, and the concrete `appendAuditRecord` facade is exposed on `core/contracts/services.ts`. **Binding (from D2/D3 drafting):** the append MUST use a **non-`RETURNING`** insert on the caller's executor (app-minted UUIDv7) — Prisma `create()` emits `INSERT … RETURNING`, which a tenant context fails against the staff-only SELECT policy (SQLSTATE 42501). Do **not** widen SELECT. |
