...
| **Status** | **OPEN — ACTIVE. D1–D3 + ADR-021 DONE; D2/D3/ADR-021 APPROVED (human) + FOLDED into the corpus (2026-06-30).** Corpus artifacts: `generatedDocs/Doc-4B_AuditAppendRLS_Patch_v1.0.1.md` · `generatedDocs/Doc-6B_Structure_Additive_Patch_v1.0.1.md` · `generatedDocs/ADR-021_Audit-Records-RLS-Asymmetry.md`, registered in `00_AUTHORITY_MAP.md`. Folded **additively** — no frozen file edited in place; carried alongside the frozen docs (Compendium-v2 / SERIES-manifest consolidation deferred to a human re-freeze). **D4 is PAUSED per owner directive (stop before D4);** D4 direction confirmed (keep SELECT, eliminate RETURNING, app-side UUID, atomic, no `SECURITY DEFINER`/elevation) and bound by the Doc-6B patch's normative Deployment Constraint. D4–D7 pending owner go-ahead. No D4 code written. |
