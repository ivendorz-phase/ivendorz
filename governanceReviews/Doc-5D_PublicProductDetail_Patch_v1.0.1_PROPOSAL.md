# Doc-5D_PublicProductDetail_Patch_v1.0.1_PROPOSAL.md

> **⏳ STATUS: PROPOSAL — awaiting API-Gov Board + human approval (CLAUDE.md §8).** Additive wire-realization
> patch; **no frozen file is edited in place.** On approval this file is copied to
> `generatedDocs/Doc-5D_PublicProductDetail_Patch_v1.0.1.md` (producing **Doc-5D v1.0.1**), registered in
> `00_AUTHORITY_MAP.md` + `CORPUS_INDEX.md`, and carried **alongside** the unedited frozen Doc-5D
> (`Doc-5D_Structure_v1.0_FROZEN.md` + `Doc-5D_Content_v1.0_Pass1…3` + `Doc-5D_SERIES_FROZEN_v1.0.md`).
>
> **Linked-pair** with `Doc-4D_PublicProductDetail_Patch_v1.0.3` (the contract this realizes) — approved and
> folded **together**, mirroring the Doc-2 v1.0.4 ↔ Doc-4C v1.0.2 linked-pair precedent.
>
...
- **API path ≠ page URL (anti-conflation note):** the public **page** canonical URL is the presentation
  URL law owned by the sibling instrument **E-3 (ADR-025-class record)** — referenced by pointer, never
  restated (the page-URL shape is deliberately not spelled here; E-3's channel owns every literal of it
  **[review-adjudicated 2026-07-03]**). This wire path is the **API** surface only; nothing here defines,
  constrains, or duplicates the page URL law.
...
| Canonical URL | `canonical_url` — **opaque, read-only; clients MUST NOT derive, modify, or reconstruct it** and MUST NOT construct product URLs locally **[owner-ruled 2026-07-03]** | K | canonical URL builder (E-3/ADR-025, by pointer; never stored) |
...
**intended POLICY identifier `marketplace.public_read_rate_limit` — reserved (unregistered); referenced by
intended name only, registration not implied** **[owner-ruled 2026-07-03: wording]**. Registration (names +
values) = sibling instrument **E-2** (additive Doc-3 §12.2 batch), required **before contract freeze** of
this realization (Doc-4A §18.2; DD-6 precedent, CHK-5A-071 family).
...
the real fields. *(Recorded for Board ratification as a refinement of the owner-approved "deepest,
specialized-first" rule — same ordering, with the actual `level` enum inserted as a late tiebreak.)*
...
**This is a conformance correction only. No new contract is introduced.** **[owner-ruled 2026-07-03: explicit
framing]**
...
| F-7 *(this patch)* | **Breadcrumb determinism**: for fixed assignment data, `primary_category_path` and `category_paths[]` order are identical across repeated reads, actors, and implementations (Part 2 clauses) |
| F-8 *(this patch)* | **Spec-entry public-leg gate**: an entry with no published-product linkage returns the uniform `NOT_FOUND` on the public leg, byte-identical to absence, while remaining readable on its Controlling-Org leg |
...
no POLICY key **registered** (intended identifier reserved only — E-2) · no error code coined · no audit
action, event, or slug touched · the granular `get_category_assignments.v1` stays vendor-addressed
Controlling-Org (**no widening** — resolution Decision 3) · out-of-wire §9 set unchanged (7 contracts) ·
no page-URL law defined (E-3) · no matching/ranking/discovery-ordering surface created (R6/DD-2).

# Downstream resolution (recorded, not edited here)

- **`Doc-4D_PublicProductDetail_Patch_v1.0.3` (linked pair):** the contract this Part 1 realizes — approved
  and folded together, or not at all.
- **E-2** (Doc-3 §12.2 batch — registers `marketplace.public_read_rate_limit` names/values) and **E-3**
  (ADR-025-class URL law + Doc-7 leg) remain **pending sibling instruments — not resolved here.**
- **Doc-8:** Part 4 rows wired at fold time (E-1 closure step; the E-3-owned rows F-2/F-4/F-5 are
  reconciled by E-3's channel if ADR-025 refines the URL law).
- **`ESC-7-API-PRODDETAIL`:** E-1 **wire leg** resolved by this patch on fold; registry pointer updated at
  fold time (E-5/E-6 channels). **FE-PUB-05** un-gates when **Annex E items 1–3** (E-1 + E-2 + E-3) have
  landed (resolution Decision 10, cited per its ratified text) **[review-adjudicated 2026-07-03]**.

---

*End of Doc-5D_PublicProductDetail_Patch_v1.0.1 (PROPOSAL) — realizes the composed Public read on
...
Coins no verb, code, slug, event, audit action, or POLICY registration. Linked-pair with
Doc-4D_PublicProductDetail_Patch_v1.0.3.*
