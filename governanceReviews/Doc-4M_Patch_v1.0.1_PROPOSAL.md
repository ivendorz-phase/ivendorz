# Doc-4M Additive Patch v1.0.1 — M5 Index Correction: Organization + Membership Rows (Editorial) — **PROPOSAL**

> **STATUS: PROPOSAL — NOT IN FORCE.** Doc-4M is FROZEN; this patch executes into
> `generatedDocs/Doc-4M_Patch_v1.0.1.md` (+ `00_AUTHORITY_MAP.md` registration) **only on a human
> owner/Board ruling** via `governanceReviews/BOARD-PACKET-IDN-4M-INDEX_v1.0.md`. Until then this
> file binds nothing. Precedent for class + mechanics: `generatedDocs/Doc-6C_Patch_v1.0.1.md`.
...
| **Authorized by** | *(pending)* Human owner/Board ruling — `ESC-IDN-4M-INDEX` (packet: `governanceReviews/BOARD-PACKET-IDN-4M-INDEX_v1.0.md`) |
...
---
*Additive patch; the frozen base file is never edited in place. Takes force only on human
owner/Board approval per CLAUDE.md §7/§8 and Doc-4M :18/:34/:351's own correction procedure.
To be registered in `generatedDocs/00_AUTHORITY_MAP.md` upon execution.*
