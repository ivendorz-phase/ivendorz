# Doc-4F Additive Patch — BC-OPS-6 Vendor Buyer Relationships (PROPOSAL)

> **PROPOSAL v1.2 — T3 of the approved domain patch train.**
> **Verification record (same Review-A agent, focused pass 2026-07-19): RA-01→12 ALL CLOSED
> against the ruled semantics; two v1.1-introduced MINORs (NF-01 §F9 completion-read
> declaration · NF-02 header amendment surface) + three NITs (NF-03 naming equivalence ·
> NF-04 grade residue · NF-05 `created_at` clause) — all applied in this v1.2. Verifier's
> conditional fold answer: YES once the Doc-4E payload patch is separately approved and
> folded. `ESC-IDN-ORG-DISPLAY-LABEL` correctly carried — does NOT gate the fold.** On owner fold-approval this becomes
> the Doc-4F additive patch (suggested fold name: `Doc-4F_BCOPS6_Additive_Patch_v1.0.md`) and is
> registered in `generatedDocs/00_AUTHORITY_MAP.md`. Until folded, nothing here is authoritative.
> **Review record (Team-4 Review-A, 2026-07-19):** RA-01 BLOCKER (VendorInvited payload gap) ·
> RA-02..05 MAJOR · RA-06..11 MINOR · RA-12 NIT — **all dispositions Board-ratified 2026-07-19**
> (RA-01 = Option (a) Doc-4E payload patch; RA-04 = Option (i) Identity display-label seam,
> carried as `ESC-IDN-ORG-DISPLAY-LABEL`; RA-11 = accept with normative-completeness condition).
> v1.1 applied all corrections. **Dependency SATISFIED (2026-07-19):** the Doc-4E
> `VendorInvited` payload patch is FOLDED as
> `generatedDocs/Doc-4E_VendorInvited_Payload_Additive_Patch_v1.0.md` (**PATCH-4E-VIP-01**,
> Authority-Map-registered) — the RA-01 fold-blocking condition is cleared.
>
...
| **Class** | **New bounded context + contract set — additive.** BC-OPS-6 (contexts 5→**6**) owning the Doc-2 v1.0.9 Vendor Buyer Relationship aggregate (M4 aggregates 7→**8**); 9 contracts (3 commands · 3 queries · 3 System consumers). No new event, no new slug, no new POLICY key, no new audit action (all bound to Doc-2 v1.0.9 / Doc-3 registrations by pointer). |
| **Domain authority** | **Doc-2 v1.0.9** (`generatedDocs/Doc-2_Patch_v1.0.9.md`, PATCH-D2-08, FOLDED 2026-07-19) — the aggregate, tables, stage set, provenance model, slug, audit rows, and consumer declarations this patch realizes. Per Doc-4A §20.1/§20.2 the domain change preceded this contract patch. |
...
| **Conforms To** | Architecture v1.0 FINAL, ADR Compendium v1, **Doc-2 v1.0.9**, Doc-3 v1.0.2 (+ POLICY registration patches v1.0–v1.11), Doc-4A v1.0, Doc-4B/4C/4D/4E v1.0, Doc-4F Structure + Pass-A + Pass-B Parts 1–5 (all FROZEN) |
...
- **BC-OPS-6 — Vendor Buyer Relationships** (Vendor Buyer Relationship aggregate): the
  ...
  transaction-axis Vendor Lead; sibling aggregates, no FK — Doc-2 v1.0.9.)*
```
...
- **Vendor Buyer Relationship** — root `vendor_buyer_relationships`; children
  ...
  (append-only); VO RelationshipStage/ProvenanceTier → **BC-OPS-6**. (Doc-2 v1.0.9)
```
...
"**8** aggregates (Doc-2 v1.0.9)". No other §F15 sentence changes.
...
- §F5 purpose sentence (:75): "the seven Module-4 aggregates" reads, per this patch, "the
  **eight** Module-4 aggregates (Doc-2 v1.0.9)".
...
- Owned: `vendor_buyer_relationships` (+`vendor_buyer_relationship_provenance`,
  `vendor_buyer_relationship_activities`) → BC-OPS-6 (Doc-2 v1.0.9 §2/§3.5/§10.5).
- Tenancy class: tenant-owned (vendor controlling org), NEVER buyer-visible; never feeds
  routing/matching/scores (Doc-2 v1.0.9 §10.5 rule 6); `buyer_organization_id` referenced by
  bare UUID only.
```
...
aggregate only (`vendor_buyer_relationships` + two append-only children — Doc-2 v1.0.9). It
...
**`can_manage_buyer_relationships`** (Doc-2 v1.0.9 §7). Audit binds the **Doc-2 v1.0.9 §9
...
(DF-2), **no** score (DF-4), and is **never buyer-visible** (Doc-2 v1.0.9 §10.5 firewall rule).
...
- **H6.3 — Authorization.** Three-layer check per Part-3 H.3; scope = the **vendor controlling
  ...
  Sole slug **`can_manage_buyer_relationships`** (O,D,M,F — Doc-2 v1.0.9 §7); reads bind the
...
- **H6.5 — State & derived value.** `stage ∈ {new, developing, active, dormant, archived}`;
  ...
  audited, **no automatic or mandatory progression** (Doc-2 v1.0.9 §3.5). `customer`, `won`,
...
- **H6.6 — Audit.** Every mutation binds the Doc-2 v1.0.9 §9 Vendor-Buyer-Relationships row —
...
- **H6.8 — Idempotency & provenance identity.** Mutations: `Idempotency: required`, window =
  ...
  dedup on event identity **plus** the Doc-2 v1.0.9 **provenance-observation identity**
...
- **H6.9 — Tenancy, non-disclosure & provenance-write authority.** Tenant-owned by the vendor
  ...
  routing/matching/scores** (Doc-2 v1.0.9 §10.5 rule 6). **Provenance-write authority (v1.0.9
...
- **H6.10 — Field source (Doc-2 v1.0.9 §10.5).** All schemas bind the three patched tables'
...
- **Server writes (never caller-supplied):** `stage = new` (v1.0.9 rule 4 — the flow never asks
...
- **Same-stage rule (RA-06, Board-ruled 2026-07-19):** `target_stage == current_stage` (with a
  ...
  **no audit row** is written, and **no timestamp changes** solely because of the no-op.
  "Every change audited" (Doc-2 v1.0.9) is satisfied: a no-op is not a change.
...
- **Archived default (RA-03):** rows with `stage = archived` are **excluded by default**;
  `filter.stage = archived` returns them (v1.0.9 reversibility). Deterministic in both
  directions.
...
v1.0.9). *Stage is never touched by a consumer* (H6.5). *Validation* = SYNTAX (malformed →
...
| § | Contract-ID(s) | Template | Owned entity (Doc-2 v1.0.9) | Actor | Emits | Consumes | Audit (§9 v1.0.9) |
|---|---|---|---|---|---|---|---|
...
v1.0.9 — none invented); audit rows enumerated (v1.0.9 §9 — none invented; tokens = T5); both
...
*Additive patch proposal; no frozen base file edited. Realizes Doc-2 v1.0.9 per Doc-4A
