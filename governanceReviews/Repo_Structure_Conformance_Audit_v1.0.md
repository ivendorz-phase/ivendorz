...
|---|---|---|
| `node scripts/check-structure.mjs` | **FAIL** — `missing dir: app/(admin)` | See F-02. Gate was GREEN at Wave-0 exit (`Wave-0_Integration_Audit_and_Exit_Gate_v1.0.md` item #4); placeholder `app/(admin)/.gitkeep` was deleted as collateral in docs commit `a9de258` and the gate has been silently RED since. |
...
| **F-02** | **MAJOR** | `app/(admin)` is absent from disk and git — deleted as collateral by docs commit `a9de258`; the ratified Wave-0 structure gate (`check-structure.mjs`) has been RED since. §8 also situates the M8 admin console in `(admin)/` while all admin pages live at `(app)/admin/` (URLs unaffected — groups are non-URL). | Two-part: (i) **restore `app/(admin)/.gitkeep`** in execution commit C2 to return the ratified gate to GREEN (mechanical restoration of a ratified state); (ii) owner ruling (G1 ask ③) on the console's constitutional home — codify the `(app)/admin/` interim in v1.1, or schedule migration into `(admin)/` in a later wave. |
