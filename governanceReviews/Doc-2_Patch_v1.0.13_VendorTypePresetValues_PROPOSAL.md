# Doc-2_Patch_v1.0.13_VendorTypePresetValues_PROPOSAL.md

> **✅ APPROVED + FOLDED 2026-07-22 (owner).** This is the authoring record + Review-A disposition. The
> folded canonical is `generatedDocs/Doc-2_Patch_v1.0.13_VendorTypePresetValues.md`; Doc-2 is at
> **v1.0.13** (`00_AUTHORITY_MAP.md` Doc-2 row). Linked-pair `Doc-4D_VendorTypePreset_Pointer_Patch_v1.0.4`
> folded together. `ESC-MKT-VENDORTYPE` values half closed. Retained as the review trail — not reopened.
>
> ---
>
> **STATUS (historical): DRAFT — ADDITIVE PATCH; AWAITS HUMAN APPROVAL.** Doc-2 is **rank-0 frozen architecture**
> (`generatedDocs/00_AUTHORITY_MAP.md:53`); a FROZEN document is changed only by additive patch +
> (rank 0–1) human approval (`CLAUDE.md:139–140`) — never a skill decision. Not folded by any AI action.
>
> **Supersedes `governanceReviews/Doc-2_Patch_v1.0.12_VendorTypePresetValues_PROPOSAL.md`** (five
> values, bound to the pre-re-freeze five-row register). v1.0.12 is retained on disk, marked
> SUPERSEDED, and **not** deleted — the review trail cites it. This revision declares **six** values,
> bound to the **now-in-force six-row register** (§2).
>
> **Depends on a landed rank-0 act.** This patch realizes the machine-readable value domain of the
> six-row preset register re-frozen by **AMD-MA-VTP-1**
> (`generatedDocs/Architecture_VendorTypePreset_Amendment_v1.0.md`), **independently verified PASS and
> IN FORCE 2026-07-22** (`generatedDocs/Architecture_VendorTypePreset_Amendment_Verification_v1.0.md`;
> `00_AUTHORITY_MAP.md` Master row). AMD-MA-VTP-1 §6 declares **no identifiers** and expressly hands
> the value domain to Doc-2 §10.3 — **this is that patch.**

## 1. Status

| Field | Value |
|---|---|
| Patch ID | **PATCH-D2-10** (same patch lineage as the superseded v1.0.11/v1.0.12 — the `vendor_type_preset` value domain — revised to six values) |
| Patches | Doc-2 (Domain Model & Database Blueprint) — FROZEN **v1.0.10 → v1.0.13** (label derivation §10) |
...
| Scope | The six-member value domain of `vendor_type_preset`, bound **by pointer** to the six rows re-frozen at `Master_System_Architecture_v1.0_FINAL.md:216–221` (AMD-MA-VTP-1, in force). **Nothing else.** |
...
| Authorized by | Owner ruling 2026-07-22 (six-row decision) + AMD-MA-VTP-1 in force. **Human approval of this patch still required** before fold. |

---

## 2. Problem — and what changed since v1.0.12
...
**What changed since v1.0.12.** v1.0.12 bound five values to the frozen five-row register and had to
argue whether minting slugs was *additive realization* or *re-keying* a frozen row. **That debate is
now closed by a landed act:** AMD-MA-VTP-1 re-froze the register at six rows, renaming rows 1 and 4 and
adding row 6, and it explicitly declares no identifiers (§6). Identity for the rows is settled at the
Architecture layer; this patch supplies the **machine-readable domain** the amendment deferred. The
question v1.0.12 wrestled with does not arise here.
...
# PATCH-D2-10 — `vendor_type_preset` value domain (§10.3), overlay
...
2. **Each value is bound by pointer, never restated** (`CLAUDE.md:193` — *"Reference-never-restate —
...
folded Doc-2 patch v1.0.4–v1.0.10.)
...
status column (`esc_registry.md:25` header: `| Handle | Scope / gap | Interim presentation |
Channel |`). It must **not** use the whole-handle ✅ mechanism seen at `esc_registry.md:96`
(`ESC-RFQ-MATCH-EVOLVE`), which closes a handle entirely — wrong for a half-closure.

**Registered-channel deviation, declared.** `esc_registry.md:92`'s Channel currently reads *"Additive
...
**`ESC-RFQ-MATCH-EVOLVE` item ③** (`esc_registry.md:96`) records *"industry/business-type ranking stays
DEFERRED pending … `ESC-MKT-VENDORTYPE`."* Closing half this handle touches that deferral's premise.
**No M3 change is authorized here**; recorded so the deferral's basis stays accurate.
...
## 9. Dependent contract update (linked pair, drafted separately)
...
`Doc-4A_Content_v1.0_Pass3.md:46` prescribes, never a restatement of the values. It carries no
...
Consumer-tolerance obligation below.)*

**Consumer-tolerance obligation.** `Doc-4A_…_Pass5.md:236` (*"New optional enum value for a filter/sort
parameter → Additive … Consumers must declare tolerance"*) requires the dependent update to carry that
declaration for `search_catalog`'s `vendor_type_preset` filter.

**Supersedes** `governanceReviews/Doc-4D_VendorTypePreset_Values_Additive_Patch_PROPOSAL.md` (7 values
incl. `other`; contradicting crosswalk; no identifiers) and
`governanceReviews/Doc-4D_VendorTypePreset_Values_Patch_v1.0.4_PROPOSAL.md` (rejected on route). The
7-value restatements in `ADR-023_…_PROPOSAL.md` (Decision item 2), `MASTER-CLASSIFICATION-DICTIONARY.md`
and `VENDOR-PROFILE-MODEL.md` are **downstream conformance tasks, not gates** — sequencing a rank-0
fold behind unratified lower-rank artifacts would invert `CLAUDE.md §7`.

---

## 10. Version label — derived from disk

`00_AUTHORITY_MAP.md` records Doc-2 folded at **v1.0.10** (`Doc-2_Patch_v1.0.10_GrowthHub`). `v1.0.11`
was consumed by the superseded five-value predecessor (committed + reviewed, **never folded**);
`v1.0.12` by its revision (committed, **never folded**, superseded here). To keep the review trail
unambiguous this revision takes the **next unused label: v1.0.13.** Patch ID stays **PATCH-D2-10** —
one patch (the `vendor_type_preset` value domain), revised across three labels (5 → 5 → 6 values).

**FOLD OBLIGATION.** On fold, the `00_AUTHORITY_MAP.md` Doc-2 row must carry **`v1.0.11` and `v1.0.12`
— reserved; superseded pre-fold; do not backfill**, mirroring the rank-1 precedent at
`ADR_Compendium_v1.md:58` (*"Number reserved. Do not backfill."*). Without this the next author reads
Doc-2 at v1.0.10 and legitimately re-mints v1.0.11. A disclosure inside a superseded artifact does not
reach them; this is an obligation of the fold.
...
| `app/(app)/_components/vendor/company/vendor-type-presets.ts` | The presentation register already carries the six rows + slugs, marked **NOT governed** pending this patch; on fold, its provenance comment is re-pointed to this patch as the governing value domain. |
...
| `src/modules/marketplace/contracts/types.ts:56` | Doc-comment states a stale `(manufacturer \| service_provider \| trader \| other)` set — **must be corrected** to the six-member domain on fold; the TS type is `string \| null` and coins nothing (raised as MINOR in `docs/backend/m2_public_read_slice_plan.md` §3.4). |
...
**FE realization beyond the register wiring is not authorized by this patch.**
...
| Gate | Status |
|---|---|
| AMD-MA-VTP-1 in force (owning register landed) | ✅ verified PASS 2026-07-22 (PV-AMD-MA-VTP-1) |
| Review-A (independent) — on this patch | ✅ **PASS 2026-07-22 — 0B / 0MAJ / 0MIN / 2NIT / 2OBS** (freeze-eligible). All four non-gating items dispositioned: two NIT anchor-precisions applied (v1.0.9 `:10`; ledger `:119,:120,:122`); OBS gate-lift note added to §8; OBS to add `Pass5:242` **declined with reason** (§9 — it is the entity-state row, off-point). No fabricated quotation or wrong-line citation found. |
| Review-B (independent) | ☐ not run |
| Human approval — rank-0 additive patch (`CLAUDE.md:139–140`) | ☐ not granted |
| Verification: base text `Doc-2_…_v1.0.2.md:731` matches disk verbatim; six sibling/domain anchors confirmed | ✅ run 2026-07-22 |
| Verification: each slug → row pointer resolves at `Master_…_FINAL.md:216–221` | ✅ run 2026-07-22 |
| Fold as `generatedDocs/Doc-2_Patch_v1.0.13_VendorTypePresetValues.md` + linked Doc-4D update (§9) + `00_AUTHORITY_MAP.md` (v1.0.11/v1.0.12 reservation) + `CORPUS_INDEX.md` | ☐ blocked |
| `esc_registry.md:92` amended row-wise (values closed, facets carve-out preserved, channel deviation declared) | ☐ blocked |
| `types.ts:56` comment corrected to the six-member domain | ☐ blocked (fold task) |
...
— this is an additive overlay proposal, human-approval gated, folded by no AI action; the Doc-2 base
file is not edited.

---

*End of Doc-2_Patch_v1.0.13_VendorTypePresetValues (PROPOSED) — supplies the closed six-member value
domain of `vendor_type_preset` in §10.3 as an additive overlay: `consultant` · `mro_retail_supplier` ·
`importer_equipment_seller` · `manufacturer_workshop` · `engineering_firm` · `service_provider`, each
bound by pointer to the six-row register re-frozen by AMD-MA-VTP-1 (in force) at
`Master_…_FINAL.md:216–221`, in the inline notation its sibling attributes use, closure carried in
freeze-level Binding rule 1 on the `Doc-6D_…Pass1.md:58` ENUM precedent. Doc-2 base unedited. No preset
added/renamed/removed/reseeded; no column, entity, state, event, permission, audit action, ownership or
DDL change. Re-keying dispositioned (§5). ESC values-half closed (§8). DRAFT — awaits HUMAN approval;
not folded by AI.*
