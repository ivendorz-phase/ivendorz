...
> The route-group set is `(public)/ · (auth)/ · (app)/ · (admin)/ · api/`. `(auth)` is authorized by pointer to `Build_Roadmap_v1.0.md` §2 (Wave-0 scaffold list). Nested route groups (e.g. `(app)/(buyer)`) and area folders (`account/`, `admin/`, `workspace/`) are **composition detail** outside constitutional scope — the binding rule remains §8's "routing and composition only." Admin-console location ruling (audit F-02): *recorded here per the owner's G1 decision — interim codification of `(app)/admin/` OR scheduled migration to `(admin)/`.* The `(admin)/` group placeholder is part of the ratified Wave-0 skeleton and MUST exist (enforced by `scripts/check-structure.mjs`).
>
> ### P-3 · `src/frontend/`
